...
Verify that LIFs are hosted on appropriate nodes and ports before you perform a switchback.
...
CAUTION: You must verify that the location of the data LIFs is correct on the new nodes before you perform a switchback. When you switchback the configuration, ONTAP attempts to resume traffic on the home port used by the LIFs. I/O failure can occur when the home port connection to the switch port and VLAN is incorrect. If you encounter any problems placing the LIFs on the correct nodes or ports, do not proceed with the switchback and contact NetApp technical support. 
