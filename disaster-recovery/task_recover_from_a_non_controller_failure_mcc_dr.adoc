...
. Run `metrocluster check show` from either side of the metrocluster and confirm that that all outputs are 'ok'. **Note:** In a non-MetroCluster IP environment,  connections should display "not-applicable" in the command output.
