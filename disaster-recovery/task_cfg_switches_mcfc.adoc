...
link:../install-fc/concept_port_assignments_for_fc_switches_when_using_ontap_9_0.html[Port assignments for FC switches when using 9.0]
...
.. On the replacement switch (FC_switch_A_1 in our example) verify that the ISL's are online:
+
`switchshow`
+
----
...
.. Follow the steps for configuring the switch in  link:../install-fc/generaltask_fcsw_cisco_configure_the_cisco_fc_switches_with_rcf_files.html[Configuring the Cisco FC switches] section, _except_ for the "Configuring zoning on a Cisco FC switch" section:
+
