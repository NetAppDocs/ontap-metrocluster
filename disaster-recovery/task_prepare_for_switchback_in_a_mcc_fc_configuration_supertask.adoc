...
xref:task_prepare_for_switchback_in_a_mcc_fc_configuration_supertask.html#configuring-a-brocade-fc-switch-after-site-disaster[Configuring a Brocade FC switch after site disaster]

xref:task_prepare_for_switchback_in_a_mcc_fc_configuration_supertask.html#configuring-a-cisco-fc-switch-after-site-disaster[Configuring a Cisco FC switch after site disaster]

xref:task_prepare_for_switchback_in_a_mcc_fc_configuration_supertask.html#port-assignments-for-fc-switches-when-using-ontap-9-0[Port assignments for FC switches when using ONTAP 9.0]

xref:task_prepare_for_switchback_in_a_mcc_fc_configuration_supertask.html#port-assignments-for-fc-switches-when-using-ontap-9-1-and-later[Port assignments for FC switches when using ONTAP 9.1 and later]
...
|===
4+| MetroCluster eight-node configuration
.2+| *FibreBridge 7500 bridge* .2+| *Port*
.2+| *Brocade 6505, 6510, or DCX 8510-8*
| *FC_switch_x_1* | *FC_switch_x_2*
.2+| bridge_x_1a
a|
FC1
...
2+| additional bridges can be cabled through port 19, then ports 24 through 47
2+|additional bridges can be cabled through port 23
|===
The following table shows bridge port usage when using FibreBridge 6500 bridges:
...
.2+| *Component* .2+| *Port* | *Brocade 6505, 6510, or DCX 8510-8*
| *FC_switch_x_1* | *FC_switch_x_2*
.6+|
controller_x_1
a|
...
a|
23
4+| Additional bridges can be attached using ports 25 through 48 following the same pattern.
|===
...
|===
2+| MetroCluster eight-node configuration
...
|===
4+| MetroCluster four-node configuration
.2+| *FibreBridge 7500 bridge* .2+| *Port* | *Cisco 9148, 9148S, or 9250i*
| *FC_switch_x_1* | *FC_switch_x_2*
.2+| bridge_x_1a
a|
FC1
...
a|
14
4+| Additional bridges can be attached to ports 15 through 32 following the same pattern.
|===
...
|===
4+| MetroCluster two-node configuration
.2+| *Component* .2+| *Port* | *Cisco 9148, 9148S, or 9250i*
| *FC_switch_x_1* | *FC_switch_x_2*
6+| controller_x_1
a|
FC-VI port a
...
16
2+|
2+| Additional bridges can be attached using ports 17 through 40 and 57 through 88 following the same pattern.
|===
[cols=4*,options="header"]
|===
4+| Cisco 9148S
...
16
2+|
2+| Additional bridges for a second DR group or second MetroCluster configuration can be attached using ports 33 through 40 following the same pattern.
|===
[col=4*,options="header"]
|===
| Cisco 9132T
.2+| *FibreBridge 7500 using two FC ports* .2+| *Port* |
| *Switch 1* | *Switch 2*
.2+| bridge_x_1a
a|
FC1
...
12
2+|
2+| Additional bridges for a second DR group or second MetroCluster configuration can be attached using the same port numbers on the second MDS module.
|===
...
23
2|
2| Additional bridges for a second DR group or second MetroCluster configuration can be attached using ports 25 through 48 following the same pattern.
|===
...
23
2+|
2+| Additional bridges can be attached using ports 25 through 48 following the same pattern.
|===
...
a|
96
.4+| Cisco 9250i with 24 port license
a|
ISL 1
a|
12
...
xref:task_complete_recovery.adoc#verifying-licenses-on-the-replaced-nodes[Verifying licenses on the replaced nodes]
