...
If the controller module fails during installation, and you have not yet issued the `metrocluster configure` command or the command has failed, you must replace the controller module. Then, you must wipe all of the controller modules (link:https://docs.netapp.com/us-en/ontap-metrocluster/install-ip/task_sw_config_restore_defaults.html[Restoring system defaults on a controller module]) and resume the configuration procedure from the beginning.
