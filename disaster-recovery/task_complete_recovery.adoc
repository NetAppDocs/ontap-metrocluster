...
If you have a MetroCluster IP configuration that was configured with ONTAP Mediator, you must remove and reconfigure the association with the mediator.
...
+
`set -privilege admin`
. For MetroCluster IP configurations using ONTAP Mediator, confirm that Mediator is up and operating.
