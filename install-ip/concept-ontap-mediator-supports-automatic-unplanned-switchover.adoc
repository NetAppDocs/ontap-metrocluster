...
* In an eight-node configuration, you can change the behavior to initiate a MAUSO either on failure of the cluster or a HA pair. For a HA pair, both nodes must fail at the same time.
