...
* ISL port requirements:
** The ISL port must be in `trunk` mode.
** ISL ports must be configured as "LACP trunk".
* The RCF configures the ISL ports:
** VLANs that are specified as “MetroCluster VLAN” are allowed.
** Native VLANs are not configured.
** The default native VLAN is not allowed. 
