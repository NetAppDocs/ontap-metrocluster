...
You can use MetroCluster IP configurations with IP-routed backend connections in two ways:

* *Inter-VLAN routing*: You can enable inter-VLAN routing and configure gateways on the switch ports.

* *Customer routers*: You can also connect the MetroCluster backend switches to your routers and configure the gateways on the switch ports.

NOTE: For either configuration, NetApp supports only NetApp-validated switches.  These switches are tested and sold by NetApp.  They are listed in the link:https://mysupport.netapp.com/NOW/products/interoperability[NetApp Interoperability Matrix Tool (IMT)] and in the link:https://review.docs.netapp.com/us-en/ontap-metrocluster_mcc991_releasebranch/install-ip/task_install_and_cable_the_mcc_components.html#cabling-the-ip-switches[Cabling the IP switches].
However, you can use switches that have not been validated by NetApp, but support will not be provided for the switch configuration.

The MetroCluster environment is configured and cabled as a standard MetroCluster IP configuration as described in link:task_install_and_cable_the_mcc_components.html[Install and cable the MetroCluster components].  When you perform the installation and cabling procedure, you must perform the steps specific to the layer 3 configuration:

*	You cable the MetroCluster switches to the routed IP network to provide the ISLs. The MetroCluster switches can be connected directly to the router or to one or more intervening switches. The VLAN must be extended to the gateway device.
