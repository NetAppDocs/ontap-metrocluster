...
* The network must meet the requirements as outlined in the sections xref:concept_prepare_for_the_mcc_installation.html#metrocluster-isl-requirements-in-shared-layer-2-networks[MetroCluster ISL requirements in shared layer 2 networks], xref:concept_prepare_for_the_mcc_installation.html#isl-cabling-requirements[ISL cabling requirements], and xref:concept_prepare_for_the_mcc_installation.html#required-settings-on-intermediate-switches[Required settings on intermediate switches].
