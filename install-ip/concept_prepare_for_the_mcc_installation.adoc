...
*	Using the `-gateway` parameter to configure the MetroCluster IP interface address with an IP gateway address.

NetApp supports only NetApp-validated switches for use in layer 3 wide-area networks.  These switches are tested and sold by NetApp.  Their cabling is documented by NetApp, and they are listed in the link:https://mysupport.netapp.com/NOW/products/interoperability[NetApp Interoperability Matrix Tool (IMT)]. NetApp-validated switches are also listed in the cabling section and supported by the RCF generator.
