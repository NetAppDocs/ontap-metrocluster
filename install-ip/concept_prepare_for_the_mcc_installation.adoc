...
NOTE: NetApp supports only NetApp-validated switches.  These switches are tested and sold by NetApp.  They are listed in the link:https://mysupport.netapp.com/NOW/products/interoperability[NetApp Interoperability Matrix Tool (IMT)] and in link:https://review.docs.netapp.com/us-en/ontap-metrocluster_mcc991_releasebranch/task_cable_ip_switches.html[Cabling the IP switches].

The MetroCluster environment is configured and cabled as a standard MetroCluster IP configuration as described in link:task_configure_the_mcc_hardware_components.html[Configure the MetroCluster hardware components].  When you perform the installation and cabling procedure, you must perform the steps specific to the layer 3 configuration:
...
*	All the ISL requirements that are described in xref:concept_prepare_for_the_mcc_installation.html#metrocluster-isl-requirements-in-shared-networks[MetroCluster ISL requirements in shared layer 2 networks] must be met, including the requirements for VLAN IDs.
