...
keywords: metrocluster, configuration, ontap, node-level, configuration, node, site, relationship, configuring, configure, similarities, difference, standard, cluster, verify, ha-config, state, component, restoring, restore, controller, module, manual, drive, pool, set, up, eight-node
...
xref:concept_prepare_for_the_mcc_installation.html#considerations-for-automatic-drive-assignment-and-adp-systems-in-ontap-9-4-and-later[Considerations for automatic drive assignment and ADP systems in ONTAP 9.4 and later]
...
xref:concept_prepare_for_the_mcc_installation.html#considerations-when-using-dedicated-ports[Considerations when using dedicated ports]

xref:concept_prepare_for_the_mcc_installation.html#considerations-when-sharing-data-ports[Considerations when sharing data ports]
...
* You must create all MetroCluster IP interface 'A' ports in the same VLAN and all MetroCluster IP interface 'B' ports in the other VLAN.  Refer to xref:concept_prepare_for_the_mcc_installation.html#Considerations-for-metrocluster-ip-configuration[Considerations for MetroCluster IP configuration].
...
Details for determining whether your system requires manual disk assignment are included in xref:concept_prepare_for_the_mcc_installation.html#considerations-for-automatic-drive-assignment-and-adp-systems-in-ontap-9-4-and-later[Considerations for automatic drive assignment and ADP systems in ONTAP 9.4 and later].
...
. To avoid possible nonvolatile memory mirroring issues, reboot each of the four nodes:
