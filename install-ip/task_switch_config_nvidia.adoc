...
+
NOTE: The following example shows how to configure the hostname (IP_switch_A_1), IP address (10.10.10.10), netmask (255.255.255.0 (24)), and gateway (10.10.10.1) depending on the NVIDIA switch firmware version.
+
[role="tabbed-block"]
====
.Cumulus 4.4.x
...
. If your configuration uses DAC cables, run the following command: 
+
----
...
. Reboot the switch after changing the DAC option on the switch ports:
+
`sudo reboot`
+
NOTE: When you change the option for multiple switch ports you only need to reboot the switch once.
