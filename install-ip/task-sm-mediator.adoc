...
NOTE: You can only configure an ONTAP Mediator that uses HTTPS by using the CLI. System Manager is not supported. For more information, refer to link:concept_mediator_requirements.html#learn-about-the-https-and-iscsi-protocols[Learn about the iSCSI and HTTPS protocols]. 
