...
NOTE: You can only configure an ONTAP Mediator that uses HTTPS by using the CLI. System Manager is not supported. For more information, refer to link:concept_mediator_requirements.html#select-a-communication-protocol-for-ontap-mediator[Select a communication protocol for ONTAP Mediator].  
