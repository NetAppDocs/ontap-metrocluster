...
summary: MetroCluster IP switches that are listed as supported switches in HardWareUniverse and provided by NetApp are NetApp-validated. Beginning with ONTAP 9.7, MetroCluster IP configurations can use switches that are not NetApp-validated provided that they are compliant with NetApp specifications. NetApp does not provide support services, troubleshooting and/or configuration for any non-validated switch. The switch must meet the requirements, and you must configure the switch using the guidelines as outlined below.
...
[.lead]
MetroCluster IP switches that are listed as supported switches in HardWareUniverse and provided by NetApp are NetApp-validated.
