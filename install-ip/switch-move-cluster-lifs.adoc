...
---
= Move cluster LIFs before a switch, RCF, or firmware change in a MetroCluster IP configuration
:icons: font
:imagesdir: ../media/
...
. Display the cluster ports that are connected to the cluster switches on each node:
+
...
+
NOTE: If you are upgrading the second fabric, you can skip this step and go to <<shut-down-ports,Step 4>>.
+
...
. [[shut-down-ports]]On cluster switch IP_switch_A1, shut down the ports connected to the cluster ports of the nodes.
+
[role="tabbed-block"]
====
.Cisco 
