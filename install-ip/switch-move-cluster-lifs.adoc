...
. On NVIDIA IP switches, complete the following steps:
+
...
. If you are using Cisco or Broadcom switches, on cluster switch IP_switch_A1, shut down the ports connected to the cluster ports of the nodes.
+
If you are using NVIDIA switches, you can skip to the <<verify_cluster_lif_migration, step to verify the cluster LIF migration>>.
+
[role="tabbed-block"]
====
.Cisco 
