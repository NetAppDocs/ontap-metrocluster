...
* You must perform this task before you upgrade or downgrade switch firmware, or replace the switch RCF.
+
...
+
For Cisco or Broadcom IP switches, you can skip to <<display_cluster_ports, the step to display the cluster ports>>
...
[[verify_cluster_lif_migration]]
. Verify that the cluster LIFs have migrated to the ports hosted on cluster switch IP_switch_A2. This might take a few seconds.
+
`network interface show -role cluster`
+
.Show example 
