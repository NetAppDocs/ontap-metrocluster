...
---
= Move cluster LIFs before a MetroCluster IP switch firmware or RCF update
:icons: font
:imagesdir: ../media/

[.lead]
When you update the switch firmware or reference configuration file (RCF) for a MetroCluster IP switch, you move cluster LIFs to the partner switch to enable non-disruptive cluster operations. 
...
+
If you are using NVIDIA switches, you can skip to the <<verify_cluster_ports_migration, step to verify the cluster LIF migration>>.
+
[role="tabbed-block"]
====
.Cisco 
...
After the cluster LIFs have moved to the other switch, you can follow the standard configuration procedures according to your switch vendor:
