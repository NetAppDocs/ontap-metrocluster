...
CAUTION: On Broadcom and Cisco switches, a new user is required for each cluster setup with log collection. For example,  MetroCluster 1, MetroCluster 2, MetroCluster 3, and MetroCluster 4 all require a separate user to be configured on the switches. Multiple SSH keys for the same user is not supported.
