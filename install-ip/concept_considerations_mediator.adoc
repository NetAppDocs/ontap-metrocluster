...
[.lead]
Beginning with ONTAP 9.7, you can use either the ONTAP Mediator-assisted automatic unplanned switchover (MAUSO) in the MetroCluster IP configuration or you can use the MetroCluster Tiebreaker software. It is not required to use the MAUSO or Tiebreaker software; however, if you choose do not use either of these services, you must link:../disaster-recovery/concept_dr_workflow.html[perform a manual recovery] if a disaster occurs.
