...
If you are using a mixture of internal and external disks, automatic assign cannot happen. In this scenario, you initialize the MetroCluster with the internal disks only and then manually connect the external shelf after you complete your installation or set up task. You must distribute the drives in the partially full shelves as described in <<populate-partially-full-shelves, How to populate partially-full shelves>>.
