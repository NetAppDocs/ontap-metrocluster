...
* If you are reusing an external disk shelf, you need to make sure that disk ownership on the external disk shelf has been removed before you attach it to the controller. Refer to link:https://docs.netapp.com/us-en/ontap/disks-aggregates/remove-ownership-disk-task.html[Remove ONTAP ownership from a disk].
