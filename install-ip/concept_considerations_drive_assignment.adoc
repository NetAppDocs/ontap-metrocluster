...
* A disk and all its partitions must be owned by nodes in the same high-availability (HA) pair. They cannot be owned by the disaster recovery (DR) partner or DR auxiliary partner.
