...
CAUTION: You cannot have both data and Flash Cache drives on the internal shelf.
...
a| 48 drives (data on the external shelf and root aggregate on the internal drives)
...
*Note:* If you are using internal and external drives, you first need to install ONTAP and configure the local cluster with only the internal drives. You then manually connect the external shelf after you complete your installation or setup task. A minimum of 12 internal drives is required for the root aggregate. The root internal drives should be placed from slot 1. For example, for 12 internal drives, use slots 1-3, 6-8, 12-14, and 18-20. FlashCache drives should be placed in slot 0 and slot 23.
