...
NOTE: If you are using a mixture of internal and external disks, you first initialize the MetroCluster with the internal disks only using ADP. You then manually connect the external shelf after you complete your installation or set up task. For both the internal and external drives, you must populate the partially fully shelves as described in <<populate-partially-full-shelves, How to populate partially-full shelves>>.
