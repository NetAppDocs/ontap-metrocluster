...
* A disk and all its partitions must be owned by nodes in the same high-availability (HA) pair. Partition or drive ownership within a single drive cannot be mixed between the local HA pair and the disaster recovery (DR) partner or DR auxiliary partner.
+
Example of a supported configuration:
+
|===
| Drive owner: | `ClusterA-Node01`
| Partition 1: | `ClusterA-Node01`
| Partition 2: | `ClusterA-Node02`
| Partition 3: | `ClusterA-Node01`
|===
 
* The ha-config state must be set to `mccip` before automatic provisioning to make sure that the correct partition sizes are selected allowing for appropriate root volume size. For more information, see link:https://docs.netapp.com/us-en/ontap-metrocluster/install-ip/task_sw_config_verify_haconfig.html[Verifying the ha-config state of components]
