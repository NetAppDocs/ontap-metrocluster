...
On AFF A800 and AFF A90 systems, after you disconnect the external shelf, the root partitions are created on the internal disks. The external shelf is partitioned when you manually connect it after completing your installation or setup. The root partitions (P3s) on the external shelf are not used until the root needs to expand.
...
You must meet the MetroCluster limits for the maximum number of supported drives and other guidelines. Refer to the https://hwu.netapp.com[NetApp Hardware Universe].

You need to make sure that disk ownership on the external disk shelf has been removed before you attach it to the controller.
...
[CAUTION] 
====
* You cannot have data aggregates and the root aggregate with Flash Cache drives on the internal shelf.
...
a| 36 drives (12 internal drives and 24 external drives – with the data aggregates on the external shelf and the root aggregate on the internal shelf)
a| The internal drives are divided into four equal groups (quarters). Each quarter is automatically assigned to a separate pool. The drives on the external shelves are divided into four equal groups (quarters). Each quarter-shelf is automatically assigned to a separate pool. 

*Notes:* 

* If you are using internal and external drives, you first need to install ONTAP and configure the local cluster with only the internal drives. You then manually connect the external shelf after you complete your installation or setup task.

*  A minimum of 12 internal drives is required for the root aggregate. The root internal drives should be placed from slot 1. For example, for 12 internal drives, use slots 1-3, 6-8, 12-14, and 18-20. 
