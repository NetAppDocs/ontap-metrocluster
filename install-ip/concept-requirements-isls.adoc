...
If you are using a QSFP-SFP adapter, some switches require ports configured in breakout speed mode, and other switches require ports configured in native speed mode. For example, Cisco switches require the port to be configured in native speed mode . Refer to the switch vendor documentation for more information.

If you are using a QSFP-SFP adapter, some switches require that ports are configured in breakout speed mode and other switches require that ports are configured in native speed mode. For example, to use a QSFP-SFP adapter on Cisco switches, the port must be configured in native speed mode. 
