...
If you are using a QSFP-SFP adapter, some switches require the ports to be configured in breakout mode, and other switches require the ports to be configured in native mode. For example, Cisco switches require the port to be configured in native mode to use a QSFP-SFP adapter. Refer to the switch vendor documentation for more information.

NOTE: If you configure an RCF, select the correct speed mode or use a port with an appropriate speed mode.  
