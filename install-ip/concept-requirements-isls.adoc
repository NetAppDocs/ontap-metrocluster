...
---
= Requirements for MetroCluster IP inter-switch links
:icons: font
:imagesdir: ../media/
...
These requirements apply to ISLs on all MetroCluster IP configurations:
...
** Beginning with ONTAP 9.18.1, platforms with a MetroCluster IP backend port speed of 100Gbps require a minimum ISL link speed of 100Gbps. Using a different ISL speed requires a Feature Policy Variance Request (FPVR). To file an FPVR, contact your NetApp sales team.
+
CAUTION: Upgrades to ONTAP 9.18.1 and later on platforms that do not currently meet the 100Gbps ISL requirement are not blocked and can proceed. However, NetApp strongly recommends that customers transition to 100Gbps ISL connectivity to maintain expected performance and availability levels.
...
// 2025, Dec 08, ONTAPDOC-3504
// 2025 Oct 23, ONTAPDOC-3396
