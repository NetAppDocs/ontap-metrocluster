...
** Beginning with ONTAP 9.18.1, platforms with a MetroCluster IP backend port speed of 100Gbps require a minimum ISL link speed of 100Gbps. Using a different ISL speed requires a Feature Variance Request (FPVR). To file an FPVR, please contact your NetApp sales team.
+
CAUTION: If you are upgrading to ONTAP 9.18.1 and your platform does not meet the 100Gbps ISL requirement, you can still proceed with the upgrade. However, NetApp strongly recommends that you transition to 100Gbps ISL connectivity to maintain expected performance and availability levels.
