...
By default, MetroCluster IP nodes access ONTAP Mediator mailbox LUNs using iSCSI sessions. ONTAP Mediator relies on the SCST package to provide iSCSI. This package is a kernel module that is compiled during installation specifically for the kernel. Any updates to the kernel requires the SCST package to be re-installed. ONTAP Mediator needs to be removed from the MetroCluster configuration before each upgrade and the kernel libraries need to be added back to the kernel after each upgrade. iSCSI sessions are authenticated using the Challenge Handshake Authentication Protocol (CHAP) with an obfuscated, machine-generated per-shared key, but are not fully encrypted.

Beginning with ONTAP 9.19.1 and ONTAP Mediator 1.12, you can configure a HTTPS-based Mediator for to monitor your MetroCluster IP configuration. HTTPS is a secure protocol for communication. Configuring a HTTPS-based Mediator has the following benefits:

* No need to remove ONTAP Mediator from the MetroCluster IP configuration before a kernel upgrade. 
...
NOTE: You can only configure a HTTPS-based ONTAP Mediator using the CLI. System Manager is not supported. 
