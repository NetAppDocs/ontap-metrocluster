...
** Rocky Linux 10 
* AWS EC2 instances use security groups to control inbound and outbound traffic. You need to explicitly allow the required ports (such as ports 22, 31784, and 3260) to enable secure communication between ONTAP Mediator and your ONTAP clusters. Refer to <<Firewall requirements for ONTAP Mediator>>.
* During installation, the AWS EC2 instance needs access to either the internet or your organization's internal package repository to download necessary software dependencies. You do this by configuring outbound rules in the security group or by setting up access to an internal repository.
* If you are using an internal repository, configure the appropriate `.repo` files and disable other repositories. Refer to link:https://docs.netapp.com/us-en/ontap/mediator/enable-access-repos-task.html[Provide repository access for ONTAP Mediator installation^] for details on the repository access required for your operating system. 
...
* The 443/https port is only required when you want to install ONTAP Mediator on an AWS EC2 instance.
* If you are installing ONTAP Mediator on an AWS EC2 instance, ensure that there are no firewalls or route table restrictions blocking communication between the EC2 instance and ONTAP cluster management or node management LIFs.
====
...
== Install or upgrade ONTAP Mediator in a MetroCluster configuration
...
Follow the steps in link:https://docs.netapp.com/us-en/ontap/mediator/workflow-summary.html[Install or upgrade ONTAP Mediator^] to complete your installation or upgrade.
