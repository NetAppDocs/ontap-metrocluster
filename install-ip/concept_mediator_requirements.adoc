...
Beginning with ONTAP Mediator 1.12.1 and ONTAP 9.19.1,  you can use the same ONTAP Mediator instance to monitor one four-node MetroCluster IP configuration and one SnapMirror active sync cluster peer relationship if the following requirements are met:

* ONTAP Mediator 1.12.1 or later is installed on a host OS running two-core CPU
* The ONTAP Mediator instance is configured with HTTPS

NOTE: Only one four-node MetroCluster IP configuration and one SnapMirror active sync peer relationship can be monitored using the same ONTAP Mediator instance.
