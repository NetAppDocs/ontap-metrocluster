...
* The connectivity between each site and the ONTAP mediator must be two separate failure domains.
...
+
Independent connectivity between each site and the ONTAP Mediator is required. A failure in one site must not interrupt the IP connectivity between the other two unaffected sites for more than one second.
