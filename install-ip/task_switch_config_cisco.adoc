...
. Download the MetroCluster IP RCF files from the https://mysupport.netapp.com/site/products/all/details/metrocluster-rcf/downloads-tab[MetroCluster RCF download page].
...
+
`no macsec keychain _keychain-name_ policy _policy-name_ fallback-keychain _fallback-keychain-name_`
+
----
...
+
`send-lifetime start-time duration _duration_`
+
----
...
. Display the keychain configuration:
+
`show keychain name`
+
----
...
. Configure one of the following ciphers:
* GCM-AES-128
* GCM-AES-256
* GCM-AES-XPN-128
* GCM-AES-XPN-256
...
+
`security-policy _security-policy_`
+
...
+
`macsec keychain _keychain-name_ policy _policy-name_ fallback-keychain _fallback-keychain-name_`
+
...
+
`config t`
.. Specify the 25-Gbps interface you want to configure:
+
