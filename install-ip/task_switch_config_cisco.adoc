...
. Generate the RCF files using the RCFfilegenerator tool.
+
link:https://mysupport.netapp.com/site/tools/tool-eula/rcffilegenerator[RcfFileGenerator for MetroCluster IP^]
+
NOTE: Modifications to the RCF files after they are generated are not supported.
