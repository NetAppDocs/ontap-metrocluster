...
---
= Configure Cisco IP switches for a MetroCluster IP configuration
:icons: font
:imagesdir: ../media/
...
* If you are using these steps as part of another procedure, such as a switch replacement or upgrade, check if you need to move the cluster LIFs to the partner switch to enable non-disruptive cluster operations. link:task_install_and_cable_the_mcc_components.html[Learn about configuring the MetroCluster IP switches].
...
This task requires file transfer software, such as FTP, TFTP, SFTP, HTTP, or SCP, to copy the files to the switches.
...
+
NOTE: You must update the switches in the following order: Switch_A_1, Switch_B_1, Switch_A_2, Switch_B_2. If your procedure requires you to move cluster LIFs, you must repeat the steps to move the cluster LIFs on the second fabric (Switch_A_2, Switch_B_2) after configuring the switches in the first fabric (Switch_A_1, Switch_B_1). Refer to link:task_install_and_cable_the_mcc_components.html[Learn about configuring the MetroCluster IP switches].

. Verify that auto-revert is enabled for the cluster LIFs:
+
`network interface show -vserver Cluster -fields auto-revert`
+
CAUTION: You must ensure that auto-revert is enabled for the cluster LIFs.

. If auto-revert is not enabled, modify all cluster LIFs to enable auto-revert:
+
NOTE: If you are performing these steps as part of another procedure, such as a switch replacement or upgrade, you might have disabled auto-revert as part of the steps to link:switch-move-cluster-lifs.html[Move the cluster LIFs].  
+
