...
+
NOTE: For a switch firmware or RCF update, you disabled auto-revert as part of the step to <<move_cluster_lifs_cisco, Move the cluster LIFs>>. If you are installing the firmware on the switch for the first time, you can skip this step.  
+
...
. If you are changing the RCF on existing switches, modify all cluster LIFs to re-enable auto-revert:
+
NOTE: For a switch firmware or RCF update, you disabled auto-revert as part of the step to <<move_cluster_lifs_cisco, Move the cluster LIFs>>. If you are installing an RCF on the switch for the first time,  you can skip this step.  
+
.. Re-enable auto-revert:
+
`network interface modify -vserver Cluster -lif * -auto-revert true`
