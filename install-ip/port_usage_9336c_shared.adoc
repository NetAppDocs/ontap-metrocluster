...
* If you are expanding a regular MetroCluster configuration that doesn't support connecting NS224 storage to the switch and you want to add a MetroCluster configuration that has switch-attached NS224 storage, review the following information:
** In the link:https://mysupport.netapp.com/site/tools/tool-eula/rcffilegenerator[RcfFileGenerator^] tool, you must select the platform that supports NS224 storage first, then select the platform that doesn't support NS224 storage.
+
This means that to generate the RCF for your configuration, you might need to first select the platform that supports NS224 storage from drop-down field 2 and then select the platform that doesn't support NS224 storage from drop-down field 1.
