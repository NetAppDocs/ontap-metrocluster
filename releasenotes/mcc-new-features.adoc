...
a|
SnapMirror cloud support for MetroCluster FlexGroup volumes
a|
SnapMirror cloud supports backup and restore operations for FlexGroup volumes in MetroCluster configurations.

link:https://docs.netapp.com/us-en/ontap/data-protection/cloud-backup-with-snapmirror-task.html#delete-a-snapmirror-cloud-relationship[Back up data to the cloud using ONTAP SnapMirror^]
a|
ONTAP 9.18.1
a|
New supported upgrade combinations for MetroCluster IP controller upgrades using `system controller replace` commands
a|
Added support for upgrading an AFF A70 to an AFF A90 system or a FAS70 to a FAS90 system using `system controller replace` commands in a MetroCluster IP configuration.

link:../upgrade/upgrade-mcc-ip-system-controller-replace.html[Upgrade controllers in four-node MetroCluster IP using switchover and switchback with "system controller replace" commands (ONTAP 9.13.1 and later)]
a|
ONTAP 9.18.1GA
a|
Flash Cache support for FAS50 systems in MetroCluster IP configurations
a|
Flash Cache is supported on FAS50 systems in MetroCluster IP configurations.

link:../install-ip/concept_considerations_drive_assignment.html#disk-assignment-on-fas50-systems-with-flash-cache[Disk assignment on FAS50 systems with Flash Cache]
a|
ONTAP 9.18.1
a|
...
MetroCluster IP support for end-to-end encryption
a|
End-to-end encryption is supported on AFF A400, AFF C400, FAS8300, and FAS8700 systems to encrypt back-end traffic, such as NVlog and storage replication data, between the sites in a MetroCluster IP configuration.
