...
a|
ONTAP 9.17.1 includes the following limit changes for four-node MetroCluster IP configurations: 

* AFF C800, AFF A800, AFF A900, AFF A90, and AFF A1K systems have increased FlexVol volume limits to 1250 volumes per node. Additionally, the following limits are updated:
** SVM limits: 64 SVMs per cluster
** LIF limits: 256 LIFs per cluster

* AFF A400, AFF C400, ASA A400, ASA C400, AFF C80, AFF A70, and AFF A50 systems have increased FlexVol volume limits. The increased limits are as follows:

** Per aggregate (single or multiple): 625
** Per node: 1250
** Per high-availability (HA) pair: 2500
** Per MetroCluster cluster: 5000
