...
=== Verifying bridge connectivity and cabling the FC-to-SAS bridges to the controller FC ports
...
. Verify that the command output shows that the bridge is connected to all of the disks and disk shelves in the stack that it are supposed to be connected.
+
[cols="30,70"]
|===
...
+
If the controller is configured with a quad-port FC adapter, make sure that the bridges at either end of the storage stack are not connected to two FC ports on the same ASIC. 
+
* Port a and port b share the same ASIC. 
+
* Port c and port d share the same ASIC.
+
For example, you should connect FC_bridge_A_1 to port a and FC_bridge_A2 to port c.
+
NOTE: Do not connect a stack of shelves to port a and port b. You should use port a and port c, or port b and port d, to avoid a single point of failure if an ASIC fails.
+
If the controller is configured with more than one FC adapter, do not cable the bridges at either end of the storage stack to the same adapter. You should distribute FC port 1 to adapter A and FC port 2 to adapter B to avoid a single point of failure.
+
