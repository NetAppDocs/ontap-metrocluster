...
Except for connecting the FC-VI ports the rest of the procedure for setting up a MetroCluster configuration with array LUNs, that are not using E-Series array LUNs, requires FC switches and are the same as using array LUNs in fabric-attached configurations.
