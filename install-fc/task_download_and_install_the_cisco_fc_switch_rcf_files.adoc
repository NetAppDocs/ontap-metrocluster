...
---
= Download and install Cisco FC switch RCF files for MetroCluster FC
:icons: font
:imagesdir: ../media/
...
. Download the Cisco FC RCFs from the https://mysupport.netapp.com/site/products/all/details/metrocluster-rcf/downloads-tab[MetroCluster RCF download page].
...
+
In this example, the `NX3232_v1.80_Switch-A1.txt` RCF is copied from the SFTP server at `10.10.99.99` to the local bootflash. You must use the IP address of your TFTP/SFTP server and the file name of the RCF that you need to install.
+
----
...
. Verify on each switch that the RCF is present in each switch's `bootflash` directory:
+
`dir bootflash:`
+
