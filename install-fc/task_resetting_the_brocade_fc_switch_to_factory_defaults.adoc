---
permalink: install-fc/task_resetting_the_brocade_fc_switch_to_factory_defaults.html
sidebar: sidebar
keywords: 
...
. Clear the configuration: `cfgClear`
+
----
...
. Set the default configuration: `configDefault`
+
----
...
+
NOTE: If the dynamic POD method is not in use (if POD method is in static) you must change the license assignment method to dynamic. Skip this step if the dynamic POD method is in use.
+
----
...
. Confirm that the default settings have been implemented: `switchShow`
. Verify that the IP address is set correctly: `ipAddrShow`
+
