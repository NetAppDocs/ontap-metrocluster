...
[CAUTION]
====
* In Fabric OS 10.x, Brocade supports only SHA-512 for SNMP authentication. ONTAP currently does not support SHA-512, with support limited to SHA2‑256.
* If your MetroCluster FC switches are running Fabric OS 10.x, you cannot configure SNMP. If you want to downgrade the switch firmware, you can follow the steps in link:../maintain/task_upgrade_or_downgrad_the_firmware_on_a_brocade_fc_switch_mcc.html[Upgrade or downgrade the firmware on a Brocade FC switch].
====
