...
summary: 'You must set up each node in the MetroCluster configuration in ONTAP, including the node-level configurations and the configuration of the nodes into two sites. You must also implement the MetroCluster relationship between the two sites. The steps for systems with native disk shelves are slightly different from those for systems with array LUNs.'
---
...
--
`snmpconfig --add snmpv3`
--
.Brocade switches (FOS 8.2 and earlier)
--
...
====
+
If you require RO access, after "User (ro):" specify the "snmpv3user". The following example uses Brocade switches:
+
----
