...
summary: 'You must set up each node in the MetroCluster configuration in ONTAP, including the node-level configurations and the configuration of the nodes into two sites.'
---
...
image::../media/workflow_high_level_node_and_cluster_configuration_software.gif["Workflow for high level node and cluster configuration"]
...
h| Cluster switch h| Host name h| IP address h| Network mask h| Default gateway
...
Example used in this procedure: site_A
...
Example used in this procedure: controller_A_1
...
Example used in this procedure: controller_A_2
...
Example used in this procedure: site_B
...
Example used in this procedure: controller_B_1
...
Example used in this procedure: controller_B_2
...
2+^| Same in both types of clusters
...
a|Create a mirrored root aggregate.
^a|Optional
^a|Required
...
a|Enable the MetroCluster configuration.
^a|Does not apply
^a|Required
...
h| Disk shelf (sample_shelf_name)... h|  Belongs to... h| And is assigned to that node's...
...
h| Disk shelf (example name)... h| At site... h| Belongs to... h| And is assigned to that node's...
...
* <<setup_ontap_2node_MCC,Setting up ONTAP in a two-node MetroCluster configuration>>

* <<setup_ontap_8node_4node_MCC,Setting up ONTAP in an eight-mode or four-node MetroCluster configuration>>
...
For more general information about setting up ONTAP, see link:https://docs.netapp.com/us-en/ontap/task_configure_ontap.html[Set up ONTAP^].
...
* <<config_LIFs_dedicated,Configuring intercluster LIFs on dedicated ports>>
* <<config_LIFs_shared_data,Configuring intercluster LIFs on shared data ports>>
...
link:concept_prepare_for_the_mcc_installation.html[Considerations when using dedicated ports]
...
.ONTAP 9.6 and later
--
Run the command: `network interface create -vserver _system_SVM_ -lif _LIF_name_ -service-policy default-intercluster -home-node _node_ -home-port _port_ -address _port_IP_ -netmask _netmask_`
--
.ONTAP 9.5 and earlier
--
Run the command:
...
link:concept_prepare_for_the_mcc_installation.html[Considerations when sharing data ports]
...
+
You can ignore the _-ipspace_ option if you are not using a custom IPspace. For complete command syntax, see the man page.
+
...
See link:https://docs.netapp.com/us-en/ontap/disks-aggregates/index.html[Disk and aggregate management^].
...
link:https://docs.netapp.com/us-en/ontap/disks-aggregates/index.html[Disk and tier (aggregate) management^]
...
+
NOTE: If you want to use a single mirrored data aggregate, then see <<step1_aggr, Step 1>> for instructions.
...
. [[step1_aggr]] Configure the MetroCluster in the following format:
+
|===

h| If your MetroCluster configuration has... h| Then do this...

a|Multiple data aggregates
a|From any node's prompt, configure MetroCluster:

`metrocluster configure node-name`

a|A single mirrored data aggregate
...
ONTAP currently supports AES-128 encryption and SHA-256.
//ontap-metrocluster/issues/26 2021.11.21

[CAUTION]
====
* In Fabric OS 10.x and later, Brocade supports only SHA-512 for SNMP authentication. ONTAP currently does not support SHA-512, with support limited to SHA‑256.
* If your MetroCluster FC switches are running Fabric OS 10.x or later, you cannot configure SNMP and you cannot add those switches to ONTAP health monitoring. If you want to downgrade the switch firmware, you can follow the steps in link:../maintain/task_upgrade_or_downgrad_the_firmware_on_a_brocade_fc_switch_mcc.html[Upgrade or downgrade the firmware on a Brocade FC switch].
====
...
+
NOTE: For EngineID, press *ENTER* to assign the default value.
+
----
...
====
+
NOTE: You should also set passwords on unused accounts to secure them and use the best encryption available in your ONTAP release.
...
NOTE: For improved security, NetApp recommends that you configure SNMPv2 or SNMPv3 to monitor the switch health. 

// 2025 Feb 27, ONTAPDOC-2082 

.About this task

These tasks apply only to systems with FC-to-SAS bridges.

Beginning in Fabric OS 9.0.1, SNMPv2 is not supported for health monitoring on Brocade switches, you must use SNMPv3 instead. If you are using SNMPv3, you must configure SNMPv3 in ONTAP before proceeding to the following section. For more details, see <<snmpv3-mcc-fc,Configuring SNMPv3 in a MetroCluster FC configuration>>.
...
NOTE: Beginning with ONTAP 9.8, the `storage switch` command is replaced with `system switch fibre-channel`. The following steps show the `storage switch` command, but if you are running ONTAP 9.8 or later, the `system switch fibre-channel` command is preferred.
...
a|9.5 and later
a|`storage bridge add -address 0.0.0.0 -managed-by in-band -name _bridge-name_`
a|
9.4 and earlier
a|
...
link:https://docs.netapp.com/us-en/ontap/cloud/high-availability-concept.html[High-availability configuration^]
...
. Use the procedures for negotiated switchover, healing, and switchback that are mentioned in the https://docs.netapp.com/us-en/ontap-metrocluster/manage/index.html[Recover from a disaster].
...
// 2026 July 02, ONTAPDOC-3961, ONTAPDOC-3963
// 2025 Aug 21, ONTAPDOC-2573
// 2024 Dec 16, ONTAPDOC-2525
// 2024 Jul 17, ONTAPDOC-1958
// 2024 May 15, GH issue 333, ONTAPDOC-1866
// 2024 Feb 22, GH issue 366
// BURT 1448684, 12 JAN 2022
// 2030-FEB-14, GH issue 138
// 2023-MAR-20, internal GH issue 221
// 2023-MAR-14, BURT 1533609
// 2025-MAY-29, ONTAPDOC-2742 - yos
