...
+
link:concept_tdm_wdm.html[Considerations for using TDM/WDM equipment with fabric-attached MetroCluster configurations]
...
+
https://mysupport.netapp.com/matrix[NetApp Interoperability Matrix Tool (IMT)]
+
In the IMT, you can use the Storage Solution field to select your MetroCluster solution. You use the *Component Explorer* to select the components and ONTAP version to refine your search. You can click *Show Results* to display the list of supported configurations that match the criteria.
...
. Set all ports to their default values:
+
`portcfgdefault`
+
...
. Set the general switch settings to default:
+
`configdefault`
+
...
. Set all ports to non-trunking mode:
+
`switchcfgtrunk 0`
+
The following example shows the command on FC_switch_A_1:
...
. On Brocade 6510 switches, disable the Brocade Virtual Fabrics (VF) feature:
+
`fosconfig options`
+
The following example shows the command on FC_switch_A_1:
...
. Clear the Administrative Domain (AD) configuration:
+
`ad options`
+
The following example shows the commands on FC_switch_A_1:
...
... Set the Advanced Performance Tuning (APT) policy to 1:
+
`aptpolicy 1`
...
... Set the Advanced Performance Tuning (APT) policy to 3:
+
`aptpolicy 3`
...
.. Change the license method to dynamic:
+
...
.. Change the existing community string:
+
`snmpconfig --set snmpv1`

.. Press *Enter* until you see  "Community (ro): [public]" text.
...
.. Enable virtual fabrics on the switch:
+
`fosconfig --enablevf`

.. Configure the system to use the same base configuration on all logical switches:
+
`configurechassis`
+
...
----
. Create and configure the logical switch:
+
`scfg --create fabricID`

. Add all ports from a blade to the virtual fabric:
+
...
link:concept_prepare_for_the_mcc_installation.html[Requirements for using a Brocade DCX 8510-8 switch]
...
If your configuration is running ONTAP 9.0 or earlier, see the link:install-fc/concept_port_assignments_for_fc_switches_when_using_ontap_9_0.html[Port assignments for FC switches when using ONTAP 9.0] section in the _Fabric-attached MetroCluster Installation and Configuration Guide_.
...
. [[step1_brocade_config]] Configure the port speed:
+
...
. Configure the trunking mode for each ISL:
+
...
----
. Enable QoS traffic for each of the ISL ports:
+
...
. Set the distance on each ISL port:
+
...
. Verify that both switches form one fabric:
+
`switchshow`
+
The following example shows the output for a configuration that uses ISLs on port 20 and port 21:
...
. [[step10_brocade_config]]Confirm the trunking of the ISLs:
+
`trunkshow`
...
----
. Repeat <<step1_brocade_config,Step 1>> through <<step10_brocade_config,Step 10>> for the second FC switch fabric.
...
. Create IP interface (ipif) addresses for the 10 Gbps ports on both switches in the fabric:
+
...
. Create the first of the two FCIP tunnels using the ports on dp0:
+
`portcfg fciptunnel`
+
...
. Verify that the FCIP tunnels were successfully created:
+
...
. Verify that all circuits were successfully created:
+
...
. Create IP interface (ipif) addresses for the 40 Gbps ports on both switches in the fabric:
+
...
. Verify that the ipif addresses were successfully created on both switches:
+
`portshow ipif all`
+
...
. Create the FCIP tunnel on both switches:
+
`portcfig fciptunnel`
+
...
. Verify that the FCIP tunnel has been successfully created:
+
`portshow fciptunnel all`
+
...
. Create an additional circuit on each switch:
+
...
. Configure the port speed for each non-E-port:
+
...
. Disable the port on which you want to configure compression:
+
`portdisable port-id`

. Enable compression on the port:
+
`portCfgCompress --enable port-id`

. Enable the port to activate the configuration with compression:
+
`portenable port-id`
...
include::../_include/brocade_zoning_dr_1_qosh1_fcvi_port_ac.csv[]
|===
// end include reference

|===

h| Zone in Fabric_1 h| Member ports
...
include::../_include/brocade_zoning_dr_1_qosh1_fcvi_port_bd.csv[]
|===
// end include reference
...
include::../_include/brocade_zoning_dr_2_qosh2_fcvi_port_ac.csv[]
|===
// end include reference
...
include::../_include/brocade_zoning_dr_2_qosh2_fcvi_port_bd.csv[]
|===
// end include reference

|===

h| Zone in Fabric_2 h| Member ports
...
h| Fabric h| Zone name h| Member ports

.3+| FC_switch_A_1 and FC_switch_B_1
a| QOSH1_MC1_FAB_1_FCVI
a| 5,0;5,1;5,4;5,5;7,0;7,1;7,4;7,5

a| QOSH2_MC1_FAB_1_FCVI ( 6510)
a| 5,24;5,25;5,28;5,29;7,24;7,25;7,28;7,29

a| QOSH2_MC1_FAB_1_FCVI (6520)
a| 5,48;5,49;5,52;5,53;7,48;7,49;7,52;7,53

.3+| FC_switch_A_2 and FC_switch_B_2
a| QOSH1_MC1_FAB_2_FCVI
a| 6,0;6,1;6,4;6,5;8,0;8,1;8,4;8,5

a| QOSH2_MC1_FAB_2_FCVI (6510)
a| 6,24;6,25;6,28;6,29;8,24;8,25;8,28;8,29

a| QOSH2_MC1_FAB_2_FCVI (6520)
a| 6,48;6,49;6,52;6,53;8,48;8,49;8,52;8,53
|===
...
include::../_include/brocade_zoning_dr_1a_grp1_top_fc1.csv[]
|===
// end include reference
...
include::../_include/brocade_zoning_dr_1a_grp1_bottom_fc1.csv[]
|===
// end include reference
...
include::../_include/brocade_zoning_dr_1a_grp2_top_fc1.csv[]
|===
// end include reference
...
include::../_include/brocade_zoning_dr_1a_grp2_bottom_fc1.csv[]
|===
// end include reference
...
include::../_include/brocade_zoning_dr_1b_grp1_top_fc1.csv[]
|===
// end include reference
...
include::../_include/brocade_zoning_dr_1b_grp1_bottom_fc1.csv[]
|===
// end include reference
...
include::../_include/brocade_zoning_dr_1b_grp2_top_fc1.csv[]
|===
// end include reference
...
include::../_include/brocade_zoning_dr_1b_grp2_bottom_fc1.csv[]
|===
// end include reference
...
a| site
a| Site on which the bridge pair physically resides.
a| A or B

a| stack group
a| Number of the stack group to which the bridge pair connects.
...
a| location in pair
a| Bridge within the bridge pair. A pair of bridges connect to a specific stack group.
a| a or b
|===
...
// [%header,format=csv]
// |===
// include::../_include/brocade_zoning_7500_dr_1a_grp1_top_fc1.csv[]
// |===
// end include reference

|===

h| FC switch h| Site h| Switch domain h| 6505 / 6510 / G610/ G620 port h| 6520 port h| Connects to...
...
a| MC1_INIT_GRP_1_SITE_A_STK_GRP_1_TOP_FC1
a| 5,2;5,6;7,2;7,6;5,8
|===

*DrGroup 1 : MC1_INIT_GRP_2_SITE_A_STK_GRP_1_BOT_FC1:*

|===

h| FC switch h| Site h| Switch domain h| 6505 / 6510 / G610 port h| 6520 port h| G620 port h| Connects to...
...
a| 7
a| 7
a| controller_B_2 port 0c
|===
|===

h| Zone in Fabric_2 h| Member ports

a| MC1_INIT_GRP_2_SITE_A_STK_GRP_1_BOT_FC1
a| 5,3;5,7;7,3;7,7;5,9
|===

*DrGroup 1 : MC1_INIT_GRP_1_SITE_A_STK_GRP_1_TOP_FC2:*

|===

h| FC switch h| Site h| Switch domain h| 6505 / 6510 / G610 h| 6520 h| G620 h| Connects to...
...
a| 2
a| 2
a| controller_A_1 port 0d
// ontap-metrocluster/issues/76
...
a| MC1_INIT_GRP_1_SITE_A_STK_GRP_1_TOP_FC2
a| 6,2;6,6;8,2;8,6;6,8
|===
...
a| MC1_INIT_GRP_2_SITE_A_STK_GRP_1_BOT_FC2
a| 6,3;6,7;8,3;8,7;6,9
|===
...
h| Zone in Fabric_1 hh| Member ports

a| MC1_INIT_GRP_1_SITE_A_STK_GRP_2_TOP_FC1
a| 5,2;5,6;7,2;7,6;5,10
|===

*DrGroup 1 : MC1_INIT_GRP_2_SITE_A_STK_GRP_2_BOT_FC1:*
...
a| MC1_INIT_GRP_2_SITE_A_STK_GRP_2_BOT_FC1
a| 5,3;5,7;7,3;7,7;5,11
|===

*DrGroup 1 : MC1_INIT_GRP_1_SITE_A_STK_GRP_2_TOP_FC2:*
...
a| MC1_INIT_GRP_1_SITE_A_STK_GRP_2_TOP_FC2
a| 6,2;6,6;8,2;8,6;6,10
|===
...
a| MC1_INIT_GRP_2_SITE_A_STK_GRP_2_BOT_FC2
a| 6,3;6,7;8,3;8,7;6,11
|===
...
a|MC1_INIT_GRP_1_SITE_B_STK_GRP_1_TOP_FC1
a| 5,2;5,6;7,2;7,6;7,8
|===

*DrGroup 1 : MC1_INIT_GRP_2_SITE_B_STK_GRP_1_BOT_FC1:*
...
a| MC1_INIT_GRP_2_SITE_B_STK_GRP_1_BOT_FC1
a| 5,3;5,7;7,3;7,7;7,9
|===

*DrGroup 1 : MC1_INIT_GRP_1_SITE_B_STK_GRP_1_TOP_FC2:*
...
a| MC1_INIT_GRP_1_SITE_B_STK_GRP_1_TOP_FC2
a| 6,2;6,6;8,2;8,6;8,8
|===
...
a| MC1_INIT_GRP_2_SITE_B_STK_GRP_1_BOT_FC2
a| 6,3;6,7;8,3;8,7;8,9
|===
...
a| MC1_INIT_GRP_1_SITE_B_STK_GRP_2_TOP_FC1
a| 5,2;5,6;7,2;7,6;7,10
|===

*DrGroup 1 : MC1_INIT_GRP_2_SITE_B_STK_GRP_2_BOT_FC1:*
...
h| Zone in Fabric_2 hh| Member ports

a| MC1_INIT_GRP_2_SITE_B_STK_GRP_2_BOT_FC1
a| 5,3;5,7;7,3;7,7;7,11
|===

*DrGroup 1 : MC1_INIT_GRP_1_SITE_B_STK_GRP_2_TOP_FC2:*
...
a| MC1_INIT_GRP_1_SITE_B_STK_GRP_2_TOP_FC2
a| 6,2;6,6;8,2;8,6;8,10
|===
...
a| MC1_INIT_GRP_2_SITE_B_STK_GRP_2_BOT_FC2
a| 6,3;6,7;8,3;8,7;8,11
|===
...
a| FC_switch_A_1 and FC_switch_B_1
a| MC1_INIT_GRP_1_SITE_A_STK_GRP_1_TOP_FC1
a| 5,2;5,6;7,2;7,6;5,8
...
a| FC_switch_A_2 and FC_switch_B_2
a| MC1_INIT_GRP_1_SITE_A_STK_GRP_1_TOP_FC2
a| 6,2;6,6;8,2;8,6;6,8
...
link:task_fcsw_brocade_configure_the_brocade_fc_switches_supertask.html[Zoning for FC-VI ports]
...
. Create the FC-VI zones on each switch:
+
...
. Configure the storage zones on each switch.
+
...
.. Create the storage zone for each switch domain in the switch fabric:
+
...
.. Create the configuration in the first switch fabric:
+
...
.. Add zones to the configuration, if desired:
+
...
. Disable the virtual fabric by entering the following command at the switch console:
+
...
. Enable encryption on all of the ISL ports:
+
