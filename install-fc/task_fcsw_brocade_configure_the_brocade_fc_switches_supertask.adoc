...
+
xref:concept_tdm_wdm.adoc[Considerations for using TDM/WDM equipment with fabric-attached MetroCluster configurations]
...
. [[step1-brocade-config,Step 1]] Configure the port speed:
+
...
. [[step10-brocade-config,Step 10]]Confirm the trunking of the ISLs:
+
`trunkshow`
...
----
. Repeat <<step1-brocade-config>> through <<step10-brocade-config>> for the second FC switch fabric.
