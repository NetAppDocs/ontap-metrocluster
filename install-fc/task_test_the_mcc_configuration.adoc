...
. Confirm the status of the `metrocluster operation`:
+
`metrocluster operation show`
+
...
. Monitor the result on the console by using the following commands:
+
...
+
`storage aggregate show``storage aggregate show -r -node node_A_2 !*root`
+
