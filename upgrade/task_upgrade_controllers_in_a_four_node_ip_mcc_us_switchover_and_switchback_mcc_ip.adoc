...
+
The intercluster LIFs on the old controllers must use a different home port than the port used for HA, cluster interconnect, or MetroCluster IP DR on the new controllers. For example, when you upgrade to AFF A90 controllers, the HA and cluster interconnect ports are e1a and e7a, and the MetroCluster IP DR ports are e2b and e3b.
+
...
| Go to...
a| Use ports e1a, e7a, e2b, or e3b as a home port a| <<switchover_upgrade_prepare_network_ports_2b,Substep b>>
a| Use a different home port a| <<switchover_upgrade_prepare_network_ports_3,Step 3>>

|===
+

.. [[switchover_upgrade_prepare_network_ports_2b]]Modify the intercluster LIFs to use a different home port:
+
...
+
. [[switchover_upgrade_prepare_network_ports_3]]Assign the home port of all data LIFs on the old controller to a common port that is the same on both the old and new controller modules.
