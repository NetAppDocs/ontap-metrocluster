...
. Modify the intercluster LIFs on the old controllers to use a different home port than the port used for HA, cluster interconnect, or MetroCluster IP DR on the new controllers.
+
For port distribution and allocation on the new nodes, refer to the https://hwu.netapp.com[NetApp Hardware Universe].
+
For example, when you upgrade to AFF A90 controllers, the HA and cluster interconnect ports are e1a and e7a, and the MetroCluster IP DR ports are e2b and e3b. 
+
If the old controllers you are upgrading from are using e1a, e7a, e2b or e3b for intercluster LIFs, you must modify the intercluster LIFs to use a different port:

.. View the intercluster LIFs:
+
`network interface show  -role intercluster`
+
If the home port of the intercluster LIFs does not conflict with the port assignment for HA, cluster interconnect or MetroCluster IP DR on the new controllers, you can skip to <<controller_upgrade_prepare_network_3,step 3>>.
+
If the home port of the intercluster LIFs is used for HA, cluster interconnect, or MetroCluster IP DR ports on the new controllers, you must follow the steps to modify the home port of intercluster LIFs to use a different network port.

.. Modify the intercluster LIFs to use a different network port:
+
`network interface modify -vserver <vserver> -lif <intercluster_lif> -home-port <port-not-used-for-ha-cluster-interconnect-or-mcc-ip-dr-on-new-nodes>`
.. Confirm that all intercluster LIFs are at the home port.
+
