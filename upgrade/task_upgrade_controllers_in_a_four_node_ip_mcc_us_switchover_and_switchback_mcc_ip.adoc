...
If you are upgrading to an AFF A70, AFF A90, or AFF A1K system, follow the steps to set the `hw.cxgbe.toe_keepalive_disable=1` bootarg and reboot each node on your existing system. 
...
. When the node boots, perform a giveback at the prompt: 
...
+
`ifgrp add-port -node <node_name> -ifgrp _<interface-group-name>_ -port _port-id_`
...
External key management
a|
`security key-manager external restore -vserver _SVM_ -node _node_ -key-server _host_name\|IP_address:port_ -key-id key_id -key-tag key_tag <node_name>`
