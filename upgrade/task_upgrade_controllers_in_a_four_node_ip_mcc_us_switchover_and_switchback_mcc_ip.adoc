...
* The MetroCluster IP switches (switch type, vendor, and model) and firmware version must be supported on the existing and new controllers you are upgrading. 
+
...
If you are upgrading to an AFF A70, AFF A90, or AFF A1K system, follow the steps to set the `hw.cxgbe.toe_keepalive_disable=1` bootarg and reboot each node in the DR group that is being upgraded.

NOTE: This task *only* applies to upgrades to an AFF A70, AFF A90, or AFF A1K system from a supported system. For all other upgrades, you can skip this task.
 ...
 ** link:../install-ip/task_switch_config_broadcom.html[Download and install the Broadcom RCF files]
...
== Remove interface configurations and uninstalling the old controllers
...
. If your upgrade requires replacement of the controller modules, for example, upgrading from an AFF 800 to an AFF A90 system, you must remove the controller module from the chassis when you replace the controller module or replace a component inside the controller module.
+
...
** link:../install-ip/task_switch_config_nvidia.html[Download and install the NVIDIA IP SN2100 switch RCF files]
...
`disk show`
+ 
The "container type" column displays "shared" in the `disk show` output if ADP is configured. If "container type" has any other value, ADP is not configured on the system.

. If the original systems were configured with partioned disks for Advanced Drive Partioning (ADP), at each of the replacement nodes' LOADER prompt, enable ADP:
+
...
`disk show -a`
+
CAUTION: Before you proceed with disk reassignement, you must verify that the pool0 and pool1 disks belonging to the node's root aggregate are displayed in the `disk show` output. In the following example output, pool0 and pool1 disks owned by node_B_1-old are listed.
+
