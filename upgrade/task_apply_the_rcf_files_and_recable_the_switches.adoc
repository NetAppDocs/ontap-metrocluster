...
[.lead]
You must apply the reference configuration files (RCFs) to reconfigure your zoning to accommodate the new nodes.

If you cannot use the RCFs to configure the switches, you must configure the switches manually. See:
...
. Locate the RCFs for your configuration.
+
You must use the RCFs for an eight-node configuration and that match your switch model.

. Apply the RCFs, following the directions on the download page, adjusting the ISL settings as needed.
...
+
The FC switch port usage must match the MetroCluster eight-node usage described in link:../install-fc/index.html[Fabric-attached MetroCluster installation and configuration] so that the RCFs can be used.
