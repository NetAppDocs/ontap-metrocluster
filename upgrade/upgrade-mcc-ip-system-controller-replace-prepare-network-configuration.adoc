...
.. If the MetroCluster interfaces are using 169.254.17.x or 18.x IP addresses, refer to <KB_link> to modify the IP addresses before you proceed with the upgrade.
+
CAUTION: Upgrading to any system not listed in the table with MetroCluster interface IP addresses that are 169.254.17.x or 18.x is not supported.
