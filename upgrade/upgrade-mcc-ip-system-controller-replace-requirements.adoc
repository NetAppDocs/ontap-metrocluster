...
CAUTION: You must perform this procedure as written each time you upgrade your controllers. When new platforms are released, there might be new requirements or steps that you need to follow for a successful upgrade. For example, upgrades to platforms introduced in ONTAP 9.15.1 or later require you to link:upgrade-mcc-ip-system-controller-replace-set-bootarg.html[Set the required bootarg] and other additional steps for a successful upgrade. 
