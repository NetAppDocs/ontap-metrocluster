...
. Perform the following on the nodes of the old DR group:
.. Confirm that aggregates are unmirrored:
`storage aggregate show is-mirrored false`
.. Verify that unmirrored aggregates do not have disks in remote plexes.
.. Remove ownership of pool1 disks or partitions that are owned by the nodes that are to be unjoined.
