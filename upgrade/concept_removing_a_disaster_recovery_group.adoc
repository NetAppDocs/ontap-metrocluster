...
`aggr status -s` 
+
All spare disks are displayed, including the pool0 and pool1 spare disks owned by the node. The output should be similar to the following example.
...
+
In the following example, the ownership of partitioned disk `0n.9P2` is removed before the ownership of the container disk `0n.9` is removed:
+
----
*> disk remove_ownership 0n.9P2
