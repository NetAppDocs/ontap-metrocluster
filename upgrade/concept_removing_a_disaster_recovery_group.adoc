...
NOTE: You use these steps during the transition and system refresh workflows. You might have already completed some steps in this procedure as part of these workflows.
...
.. Remove ownership of pool1 disks on each DR group node to be deleted. Perform these steps on each node to be removed. In the examples used in this step, node_A_1 is being removed.
+
... Go to the nodeshell:
+
`run -node <node_name>`
...
`disk show` 
+
All disks are displayed, including the Pool1 disks owned by the node. The following example shows truncated output where disk 0n.4 and its partitions are assigned to Pool1. The output will vary depending on your configuration.
+
----
...
... Remove disk ownership for each pool1 disk:
+
...
+
In the following example, the ownership of the partitioned disks is removed before the ownership of the container disk `0n.4` is removed:
+
----
