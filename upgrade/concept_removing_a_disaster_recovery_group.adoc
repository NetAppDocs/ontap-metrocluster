...
NOTE: Root aggregates are not deleted.
 ...
 .. Migrate all NAS data LIFs used for NFS and CIFS (SMB) to home nodes in another DR group.
 +
...
. If in a MetroCluster IP configuration, perform the following steps on the nodes of the old DR group: 
+
...
.. Capture a list of remote disks owned by the nodes in the DR group.
+
...
+
For partitioned disks, after removing partition ownership, remove the container disk ownership.
