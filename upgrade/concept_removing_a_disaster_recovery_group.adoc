...
+
You must first run the following command for each partitioned disk, and then remove the container disk ownership:
+
`disk remove_ownership <disk_name>`
+
In the following example, ownership of partitioned disk `0n.9P2` is removed before the ownership of the container disk `0n.9` is removed:
+
----
*> disk remove_ownership 0n.9P3
