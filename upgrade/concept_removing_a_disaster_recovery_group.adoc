...
. Prepare for the removal of the DR group, if you haven't already.
 ...
 .. Re-create load-sharing mirror volumes. If the DR group to be removed contains any load-sharing mirror volumes you must create a new load-sharing mirror in another DR group. Load-sharing mirror volumes cannot be moved.
