...
+
NOTE: The `metrocluster remove-dr-group` command is supported only on ONTAP 9.8 and later.
+
...
+
In a MetroCluster FC configuration, go to the step for <<fc_step_7_remove_nodes, removing the old DR group nodes>>.
+
...
.. Remove ownership of pool1 disks on each DR group node to be deleted. Perform these steps on each node to be removed. In the examples used in this step, node_A1 is being removed.
+
... Go to the nodeshell:
+
`run -node <node_name>`
...
`aggr status -s` 
+
All spare disks are displayed, including the pool0 and pool1 spare disks owned by the node. The following example shows truncated output where 0n.7 disks are assigned to Pool1. The output will vary depending on your configuration.
+
----
0n.7         node_A1(538300171)    Pool1  S7J8NE0W600508        node_A1(538300171)
0n.7P1       node_A1(538299967)    Pool1  S7J8NE0W600508NP001   node_A2(538299967)
0n.7P2       node_A1(538300171)    Pool1  S7J8NE0W600508NP002   node_A1(538300171)
0n.7P3       node_A1(538300171)    Pool1  S7J8NE0W600508NP003   node_A1(538300171)
----

... Set the privilege level to advanced:
+
`set -privilege advanced -prompt-timestamp`

... Remove disk ownership for each pool1 spare disk:
+
If your configuration contains partitioned disks, you first remove ownership of each partitioned disk, and then remove the container disk ownership:
+
`disk remove_ownership <disk_name>`
+
In the following example, ownership of the partitioned disks are removed before the ownership of the container disk `0n.7` is removed:
+
----
*> disk remove_ownership 0n.7P1
...
Do you want to continue(y/n)? y
*> disk remove_ownership 0n.7P2
Volumes must be taken offline. Are all impacted volumes offline(y/n)?? y
Removing the ownership of aggregate disks may lead to partition of aggregates between high-availability pair.
Do you want to continue(y/n)? y
*> disk remove_ownership 0n.7P3
Volumes must be taken offline. Are all impacted volumes offline(y/n)?? y
Removing the ownership of aggregate disks may lead to partition of aggregates between high-availability pair.
Do you want to continue(y/n)? y
*> disk remove_ownership 0n.7
Volumes must be taken offline. Are all impacted volumes offline(y/n)?? y
*>
----
...
. [[fc_step_7_remove_nodes]]Remove the nodes in the old DR group.
+
...
.Related information 
Learn more about the commands used in this procedure in the link:https://docs.netapp.com/us-en/ontap-cli/[ONTAP command reference^].
