...
Removing a DR Group leaves four nodes in the configuration.
...
. Prepare to remove the DR group if you have not already.
 ...
 .. If the DR group to be removed contains load-sharing mirror volumes, they cannot be moved.  Re-create all load-sharing mirror volumes in another DR group and then delete the load-sharing mirror volumes in the DR group to be removed.
 ...
 .. Delete all data aggregates in the DR group you want to remove, as shown in the following example:
+
----
...
+
`network interface migrate -vserver <svm-name> -lif <data-lif> -home-node <new_node> -home-port <port-id>`
...
* FCP interfaces used for block access (SAN) cannot be migrated or moved between the nodes. Create new FCP interfaces as needed.
...
. If in a MetroCluster IP configuration, perform the following steps to delete the remote plexes of the root aggregates and remove disk ownership on the nodes of the old DR group.
+
...
+
NOTE: Use a comma-separated list to specify the node names for the `-owner <node_names_in_the_DR_group_to_be_deleted>` field.
+
[role="tabbed-block"]
====
...
+
`storage disk show -pool Pool1 -owner <node_names_in_the_DR_group_to_be_deleted> -partition-ownership`
--
.Whole disks:
--
. Set the privilege level to advanced:
...
. Display the remote disks:
+
`storage disk show -pool Pool1 -owner <node_names_in_the_DR_group_to_be_deleted>`
--
====
