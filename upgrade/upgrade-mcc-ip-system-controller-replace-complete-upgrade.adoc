...
+
CAUTION: When there are different platform models at each site, it can cause some components of the MetroCluster check to fail. When you repeat the automated controller replacement operation to upgrade the controllers at the second site, you must skip the MetroCluster checks by using the  `- skip-metrocluster-check true` parameter  with the `system controller replace start -nso true` command. 
+
