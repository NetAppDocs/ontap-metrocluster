...
NOTE: If required, you must set the bootarg on all of the old nodes at both sites before you start the upgrade. 
...
* You *must* set a bootarg on the old controllers for most upgrades to the following systems: 
...
CAUTION: If your upgrade isn't affected, you can skip this task and go directly to link:upgrade-mcc-ip-prepare-system.html[Prepare the system for upgrade].
...
a| All other upgrades to AFF A70, AFF A90, AFF A1K, FAS70, FAS90, AFF C80, AFF A50, AFF A20, AFF A30, AFF C30, AFF C60, or FAS50 systems | `hw.cxgbe.toe_keepalive_disable`
|===


== Step 3: Set the required bootarg on the old controllers

Follow the steps to the required bootarg on the old controllers before you start the upgrade. This task is *only* required for affected upgrades.
