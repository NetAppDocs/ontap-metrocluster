...
== Start the controller replacement operation and perform system prechecks (if required)
...
+
Run the command to *Upgrade without MetroCluster prechecks* under the following circumstances:
+
...
[role="tabbed-block"]
====
.Upgrade with MetroCluster prechecks
--
...
--
.Upgrade without MetroCluster prechecks
--
. The following command skips the prechecks to avoid an error caused by having two different platform models at each site or if the configuration has unmirrored aggregates:
+
`system controller replace start -nso true -skip-metrocluster-check true`
+
CAUTION: Don't skip the MetroCluster prechecks if you are performing this procedure on the first site or if your configuration doesn't have unmirrored aggregates. 
