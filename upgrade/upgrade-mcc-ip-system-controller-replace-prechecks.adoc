...
This task is required under the following conditions: 

* If the switch RCF configuration is not on the minimum version
* If you need to change VLAN IDs used by the back-end MetroCluster connections.

NOTE: The switches at site_A will be upgraded when the controllers on site_A are upgraded.
...
* If both of your old and new platform models are in the following list, you do not need to update the VLAN ID before you upgrade controllers, the VLAN ID is configured when the interfaces are created and cannot be changed: 
...
+
If either of your old or new platform models are not listed above, you need to confirm that the MetroCluster interfaces are using a supported VLAN ID. Supported VLAN IDs for the MetroCluster interfaces are: 10, 20, or in the range of 101 to 4096.
+
[CAUTION] 
====
