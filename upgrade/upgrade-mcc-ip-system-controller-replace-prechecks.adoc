...
+
CAUTION: If you are repeating this procedure after already replacing the controllers at site B, you must run the `system controller replace start -nso true -skip-metrocluster-check true` command instead. This avoids precheck errors that can occur when there are two different platform models at each site.
+
