...
NOTE: For supported upgrades using `system controller replace` commands, refer to the table in link:task_upgrade_controllers_system_control_commands_in_a_four_node_mcc_ip.html[Upgrade controllers in a four-node MetroCluster IP configuration using switchover and switchback with "system controller replace" commands (ONTAP 9.13.1 and later)]
...
image::../media/metrocluster_fc_upgrade_table_asa.png[]
