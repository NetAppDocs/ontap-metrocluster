...
Select your current platform from the FC or IP table below. If your platform does not appear in the 'current' column, a controller upgrade from your platform is not supported.
...
*Note 3: For this upgrade refer to this procedure, For this upgrade use the procedure,* link:task_upgrade_A700_to_A900_in_a_four_node_mcc_fc_us_switchover_and_switchback.html[Upgrade controllers from AFF A700/FAS9000 to AFF A900/FAS9500 in a MetroCluster FC configuration using switchover and switchback (ONTAP 9.10.1 or later)]
