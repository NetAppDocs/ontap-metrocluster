...
image::../media/workflow_ip_upgrade.png["Workflow for upgrading controllers in MetroCluster IP configuration"]
...
+
You'll also need the most up-to-date Onboard Key Manager passphrase later in the upgrade procedure. 
+
...
--
Gather the external key management information that you'll need to restore the key manager configuration later in the procedure.

.. Display the name of the admin SVM (cluster). 
+
`vserver show -type admin`
+
In this example, the admin SVM name is cluster_B:
+
----
...
+
NOTE: The admin SVM can have up to four primary key servers but you'll only need the port number and address of one of the primary key servers to restore the key management configuration later in the procedure. If there is a username and password associated with the primary key server you choose, you'll also need that information for the recovery process. 
+
...
.. Obtain the keystore UUID for the admin SVM.
+
...
* The keystore UUID for the admin SVM
...
Tiebreaker
a|
link:../tiebreaker/concept_configuring_the_tiebreaker_software.html#remove-metrocluster-configurations[Removing MetroCluster Configurations] in the _MetroCluster Tiebreaker Installation and Configuration content_
...
image::../media/mcc_upgrade_cluster_a_in_switchover_A900.png["Site_B inactive and ready for upgrade after MetroCluster switchover"]
...
+
image::../media/drw_9500_remove_PCM.png["Removing controller module from chassis: unlock and slide out"]
+
|===
...
+
image::../media/drw_a900_move-remove_NVRAM_module.png["Removing NVRAM module from chassis: unlock and pull tabs"]
+
|===
...
+
image::../media/drw_a900_move-remove_NVRAM_module.png["Installing NVRAM module into chassis: align and lock in place"]
+
|===
|image:../media/number1.png[number 1] |Lettered and numbered I/O cam latch
|image:../media/number2.png[number 2] |I/O latch completely unlocked
...
+
image::../media/drw_9500_remove_PCM.png["Installing controller module into chassis: align, seat, and cable"]
+
|===
...
+
NOTE: Slot 7 on all nodes of the cluster should be empty as mentioned in the <<upgrade_a700_a900_ip_map,Map ports from the old nodes to the new nodes>> section.
...
+
NOTE: You must reboot the node in order to use the newly installed software. 
...
*Note:* You'll need the Onboard Key Manager backup and passphrase that you gathered earlier in the procedure.
a|
External key management
a|
Option `11`

Follow the prompts to provide the following inputs to recover and restore the key manager configuration:
...
. If root volume encryption is used, select the boot menu option again for your key management configuration.
+
|===
...
* See the port mapping plan you created in <<upgrade_a700_a900_ip_map,Map ports from the old nodes to the new nodes>>
...
image::../media/mcc_upgrade_cluster_a_switchback_A900.png["Site_A awaiting upgrade after MetroCluster switchback"]
...
. Repeat the steps to upgrade the nodes on site_A, beginning with <<upgrade_a700_a900_ip_prepare,Prepare for the upgrade>>.
+
...
Tiebreaker
a|
link:../tiebreaker/concept_configuring_the_tiebreaker_software.html#add-mcc-config-tb[Adding MetroCluster configurations] in the _MetroCluster Tiebreaker Installation and Configuration_ section.
a|
Mediator
a|
link:../install-ip/concept_mediator_requirements.html[Configure ONTAP Mediator from a MetroCluster IP configuration] in the _MetroCluster IP Installation and Configuration_ section.
a|
Third-party applications
a|
Refer to the product documentation.
