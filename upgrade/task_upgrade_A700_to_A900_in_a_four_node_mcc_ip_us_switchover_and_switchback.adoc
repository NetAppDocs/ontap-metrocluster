...
CAUTION: After you upgrade the controllers at the first site and the second site is not yet upgraded, running `metrocluster check run` followed by `metrocluster check show` returns an error in the `config-replication` field which indicates a NVRAM size mismatch between the nodes at each site. This error is expected behavior when there are different platform models on both sites. You can ignore the error until the controller upgrade is completed for all nodes in the DR group.
