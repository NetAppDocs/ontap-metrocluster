...
summary: 'Update the switch RCFs for the new platforms and set the MetroCluster IP bootarg values on the controller modules.'
---
...
In this task, you use the UUIDs and system IDs identified earlier in the upgrade procedure in link:upgrade-mcc-ip-system-controller-replace-prechecks.html#gather-information-before-the-upgrade[Gather information before the upgrade].
...
. At the `LOADER` prompt for the new nodes, set the UUIDs:
+
...
. If the original systems were configured for ADP, enable ADP at the `LOADER` prompt for each replacement node:
+
...
.What's next?
link:upgrade-mcc-ip-system-controller-replace-reassign-root-agg.html[Reassign the root aggregate disks].
