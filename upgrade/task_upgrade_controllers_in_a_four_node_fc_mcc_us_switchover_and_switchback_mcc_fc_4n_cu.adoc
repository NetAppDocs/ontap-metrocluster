...
* Controller upgrades from AFF A700/FAS9000 controllers to AFF A900/FAS9500 controllers are not shown in this table. That combination is supported using the link:task_upgrade_A700_to_A900_in_a_four_node_mcc_fc_us_switchover_and_switchback.html[AFF A700 / FAS9500 switchover/switchback procedure].
* Controller upgrades from AFF A300 or FAS8200 platforms using onboard ports 0e and 0f as FC-VI connections are supported only on systems running ONTAP versions prior to 9.9.1. For more information, review the link:https://mysupport.netapp.com/site/bugs-online/product/ONTAP/BURT/1507088[Public Report].
====
...
.. Select the option for your configuration and issue the correct command on cluster_A:
+
[role="tabbed-block"]
====
.Four- or eight-node FC configuration running ONTAP 9.8 or later
--
Run the command: `metrocluster switchover -controller-replacement true`
--
.Two-node FC configuration running ONTAP 9.3 and later
--
