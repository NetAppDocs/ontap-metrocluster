...
+
Operating the MetroCluster configuration with different controller types at one site is not supported  outside of this maintenance activity. However, for eight-node MetroCluster configurations, you can upgrade the controller for one DR group and leave the second DR group on a different platform.
