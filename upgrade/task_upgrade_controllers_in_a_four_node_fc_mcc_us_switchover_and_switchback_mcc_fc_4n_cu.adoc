...
+
NOTE: If the original or new platforms are FAS8020 or AFF8020 systems using ports 1c and 1d in FC-VI mode, review link:https://kb.netapp.com/Advice_and_Troubleshooting/Data_Protection_and_Security/MetroCluster/Upgrading_controllers_when_FCVI_connections_on_existing_FAS8020_or_AFF8020_nodes_use_ports_1c_and_1d[NetApp Knowledge Base: Upgrading controllers when FCVI connections on existing FAS8020 or AFF8020 nodes use ports 1c and 1d. ^]
...
+
Operating the MetroCluster configuration with different controller types is not supported at one site outside of this maintenance activity. However, for eight-node MetroCluster configurations, you can upgrade the controller for one DR group and leave the second DR group on a different platform.
