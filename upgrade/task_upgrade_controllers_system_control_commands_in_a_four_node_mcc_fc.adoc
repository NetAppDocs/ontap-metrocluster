...
The following table shows which controller model combinations are supported.
...
* Upgrades from AFF A700/FAS9000 controllers to AFF A900/FAS9500 controllers are not shown in this table. That combination is supported using the link:task_upgrade_A700_to_A900_in_a_four_node_mcc_fc_us_switchover_and_switchback.html[AFF A700 / FAS9500 switchover/switchback procedure].
* Upgrades from AFF A300 or FAS8200 platforms using onboard ports 0e and 0f as FC-VI connections are supported only on systems running ONTAP versions prior to 9.9.1. For more information, review the link:https://mysupport.netapp.com/site/bugs-online/product/ONTAP/BURT/1507088[Public Report].
====

image::supported_controller_upgrades_mccfc_sosb.png[supported controller upgrade combinations in FC configurations]
...
configuration is successfully resynchronized on the partner cluster: `-simulate true`
