...
---
= Using "system controller replace" commands to upgrade controllers in a MetroCluster FC configuration (ONTAP 9.10.1 or later)
:icons: font
:imagesdir: ../media/
...
* This automated switchover based controller upgrade procedure gives you the capability to initiate controller replacement to a MetroCluster donor site. You can only initiate a controller replacement at one site at a time.
...
. If required, manually modify the ports for the cluster LIFs and broadcast domain details by using manual commands.
...
. The automation process pauses at the network reachability task status so you can reconfigure VLANs and interface groups. If required, manually modify the ports for the cluster LIFs and broadcast domain details before resuming the operation.
...
`system controller replace show`
+
If the post upgrade checks did not report any errors, the automation process is complete.
