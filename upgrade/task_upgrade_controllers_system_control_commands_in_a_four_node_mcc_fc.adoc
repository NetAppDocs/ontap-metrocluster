...
* After the automation procedure completes the prechecks, the current source system and all compatible target systems are displayed. If you have replaced the source controller with a controller that has a different ONTAP version or a non-compatible platform, the automation procedure halts and reports an error after the new nodes are booted up. To bring the cluster back to a healthy state, you need to follow manual recovery procedure
...
At any stage during the upgrade, you can run the `system controller replace show` or `system controller replace show-details` commands from site A to check the status. If the commands return a blank output, wait for a few minutes, and then rerun the command.
...
The automation procedure runs verification system checks and then pauses so you can verify the network reachability. After verification, the resource regain phase is initiated and the automation procedure executes switchback at site A and pauses at the post upgrade checks. After you resume the automation procedure, it performs the post upgrade checks and, if no errors are detected, marks the upgrade process as complete.
...
+
`system controller replace resume`
.	The automation process performs switchback at site A and post upgrade checks. When the process pauses the post upgrade checks, manually check the SAN LIF status and verify the network configuration by following the *onbox message*.
.	After you complete the verification, resume the operation:
+
`system controller replace resume`
