...
+
The `system controller replace start` command performs MetroCluster system health checks that run in the background. In certain circumstances, NetApp recommends to skip the MetroCluster system health check to avoid unnecessary warnings. 
+
You only run the command to *Upgrade without MetroCluster checks* under the following circumstances:
+
* Your configuration has unmirrored aggregates. 
* You are repeating the procedure at one site after already replacing the controllers at the other site.
+
[role="tabbed-block"]
====
.Upgrade with MetroCluster checks
--
...
--
.Upgrade without MetroCluster checks
--
. Skip the checks to avoid an error caused by having two different platform models at each site or if the configuration has unmirrored aggregates:
+
`system controller replace start -skip-metrocluster-check true`
+
CAUTION: Don't skip the MetroCluster checks if you are performing this procedure on the first site or if your configuration doesn't have unmirrored aggregates. 
