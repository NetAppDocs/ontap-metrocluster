...
[NOTE]
====
If you have unmirrored aggregates, you cannot proceed with the upgrade and the `system controller replace start` command reports the following precheck error:
----
...
----
If this error occurs, verify that all mirrored aggregates are healthy and not degraded or mirror-degraded. You can override this error by selecting the `-skip-metrocluster-check` true option on the `system controller replace start` command. If remote storage is accessible, the unmirrored aggregates comes online after switchover. If the remote storage link fails, the aggregates fails to come online.
====
