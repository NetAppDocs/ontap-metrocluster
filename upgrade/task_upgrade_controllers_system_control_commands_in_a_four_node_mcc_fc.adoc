...
Refer to link:concept_choosing_an_upgrade_method_mcc.html[Choosing an upgrade or refresh method] for additional procedures.
...
.	Start the automated controller replacement operation.
+
The `system controller replace start` command performs MetroCluster system health checks that run in the background. If you're repeating the procedure at one site, after already replacing the controllers at the other site, NetApp recommends that you skip the checks to avoid warnings that occur when there are different platform models at both sites.
+
[role="tabbed-block"]
====
.If you're upgrading the first site
--
...
--
.If you're upgrading the second site
--
.. Run the command to start the upgrade without MetroCluster checks:
+
`system controller replace start -skip-metrocluster-check true`
+
CAUTION: Don't use the `-skip-metrocluster-check true` parameter when all of the nodes in the DR group are of the same platform model. 
...
+
CAUTION: When there are different platform models at each site, it can cause some components of the MetroCluster check to fail. When you repeat the automated controller replacement operation to upgrade the controllers at the second site, you must skip the MetroCluster checks by using the  `system controller replace start -skip-metrocluster-check true` command. 
+
