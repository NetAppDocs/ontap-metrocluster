...
.	Update the RCF on the fabric A switches, switch A1, and switch B1 by following the steps in link:disaster-recovery/task_cfg_switches_mcfc.html[Configuring the FC switches].
.	Re-cable the existing FC-VI and storage connections on the fabric A switches (switch 1 and 3) as directed by the new RCF, refer to MetroCluster installation and configuration section for the AFF A900 cabling.
...
.	After re-cabling the fabric A connections as directed by the new RCF, all storage and FC-VI connections should come online. Check the FC-VI connections:
+
...
`metrocluster check show`
.	Update and re-cable the fabric B connections by repeating *xref steps 2 to 4*.
...
+
image:drw_9500_remove_PCM.png[controller]
+
|===
...
+
image:drw_a900_move-remove_NVRAM_module.png[NVS module]
+
|===
