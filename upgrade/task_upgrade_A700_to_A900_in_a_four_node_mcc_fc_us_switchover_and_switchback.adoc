...
[NOTE] 
====
* If there are any add-on modules used as coredump devices on the AFF A700 non-volatile storage module in slot6, do not transfer those to the AFF A900 NVS module. Do not transfer any parts from the AFF A700 controller module and NVS to the AFF A900 controller module.
* For FAS9000 to FAS9500 upgrades, only Flash Cache modules on the FAS9000 NVS module should be transferred to FAS9500 NVS module. Do not transfer any other parts from the FAS9000 controller module and NVS to the FAS9500 controller module. 
====
...
`metrocluster check show`
+
After running the `metrocluster check run` and `metrocluster check show` commands, you might see an error similar to the following: 
+
----
...
----
+ 
This error occurs due to controller mismatch during the upgrade process. You can safely ignore the error and proceed to upgrade the nodes on site_A. 
