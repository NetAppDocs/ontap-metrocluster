...
* You can use this procedures with ONTAP 9.10.1 and later.
...
.	Re-cable the existing FC-VI and storage connections on the fabric A switches (switch 1 and 3) as directed by the new RCF, refer to MetroCluster installation and configuration section for the AFF A900 cabling *[Is this just a link to the Fabric ICG?]*.
+
...
*[Do we need to include additional details on checking connections after RCF change+connection move? Also is an example output for this step a good idea?]*
...
== Remove the controller module and NVS from both nodes at site A
...
NOTE: If there are any add-on modules used as coredump devices on the AFF A700 non-volatile storage module, do not transfer those to the AFF A900 NVS. Do not transfer any parts from the AFF A700 controller module and NVS to the AFF A900. *[delete last sentence?]*

== Install the AFF A900 NVS and controller module

You must install the AFF A900 NVS and controller module from the upgrade kit on both nodes at site_A. Do not move coredump device from AFF A700 NVS module to AFF A900 NVS module.
...
It is not possible to check the ONTAP version installed on the boot media of an AFF A900 controller module unless it is installed in a chassis and powered ON. The ONTAP version on the AFF A900 boot media should must be same as the ONTAP version running on the AFF A700 system that is being upgraded and both the primary and backup boot images should match. You can configure the images by performing a `netboot` followed by the `wipeconfig` command from the boot menu. If the controller module was previously used in another cluster, the `wipeconfig` command clears any residual configuration on the boot media.

.Before you start

•	Verify that you can access a HTTP server with the system.
•	You need to download the necessary system files for your system and the correct version of ONTAP from the XREF NetApp Support Site.
About this task
You must netboot the new controllers if the version of ONTAP installed is not the same as the version installed on the original controllers.if they do not have the same version of ONTAP 9 installed on them that is installed on the original controllers. After you install each new controller, you boot the system from the ONTAP 9 image stored on the web server. You can then download the correct files to the boot media device for subsequent system boots.
...
`disk show -a`
+
The command output shows the system ID of the new controller module (1574774970). However, the root aggregate disks are still owned by the old system ID (4068741254). This example does not show drives owned by other nodes in the MetroCluster configuration. *[Rakesh to provide a new sys ID]*
+
----
*> disk show -a
...
|===
+
You might need to issue the `recover_xxxxxxxx_keymanager` command and select option `6` at the boot menu prompt multiple times until the nodes completely boot.
