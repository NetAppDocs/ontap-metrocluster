...
+
link:../install-ip/task_cable_ip_switches.html[Cabling the IP switches]
...
NOTE: You can configure the MetroCluster IP interfaces from either cluster.  Also, starting with ONTAP 9.1.1, if you are using a layer 3 configuration, you must also specify the `-gateway` parameter to create MetroCluster IP interfaces. Refer to link:../install-ip/concept_prepare_for_the_mcc_installation.html#Considerations-for-layer-3-wide-area-networks[Considerations for layer 3 wide-area networks].
