...
* The table shows all platform models that use *shared MetroCluster/HA ports* and *shared cluster/HA ports*. Not all combinations are supported by this procedure. Refer to the <<expand-ip-supported-platforms, supported platform combinations>> to verify if your expansion combination is supported.
==== 
...
The following tables show the supported platform combinations for eight-node MetroCluster IP configurations.
...
image:../media/expand-ip-group-1-updated.png["Expansion combinations for AFF and FAS group 1 platforms in MetroCluster IP"]
--
...
image:../media/expand-ip-group-2-updated.png["Expansion combinations for AFF and FAS group 2 platforms in MetroCluster IP"]
--
==== 
...
// 2025 Mar 24, ONTAPDOC-2840, FAS50
...
. From Maintenance mode, issue the halt command to exit Maintenance mode:
+
`halt`

. Check if the partner-sysid is correct:
+
...
. Boot the system and get to cluster setup:
+
`boot_ontap`
+
...
`network port show`
+
For complete command syntax, see the link:https://docs.netapp.com/us-en/ontap-cli[ONTAP command reference^].
+
...
+
The following example assigns ports "e0e" and "e0f" to failover group "intercluster01" on SVM "cluster01":
+
----
...
+
The following example shows that the intercluster LIFs "cluster01_icl01" and "cluster01_icl02" on port "e0e" will fail over to port "e0f".
+
----
...
You must create the MetroCluster interfaces on the new MetroCluster IP nodes and mirror root aggregates.
...
. On each site, add the new intercluster LIFs to the peer relationship.
+
From cluster_A, run the following command:
+
...
+
From cluster_B, run the following command:
+
...
+
NOTE: The output of the `metrocluster configuration-settings connection show` command can vary depending on your platform model. For example, on some platforms, the lines for HA Partner are not displayed in the output. 
+
----
...
.. Refresh the MetroCluster configuration on any of the newly added nodes:
+
[cols="30,70"]
|===
...
// 2026 June 10, ONTAPDOC-3805
// 2025 Aug 28, ONTAPDOC-3293
// 2024 Feb 20, ONTAPDOC-2807
