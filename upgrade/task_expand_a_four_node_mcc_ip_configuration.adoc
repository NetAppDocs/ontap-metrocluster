...
. Cable the new MetroCluster IP nodes to the IP switches as shown in link:../install-ip/using_rcf_generator.html[Cabling the IP switches].
...
+
For example, you have a four-node MetroCluster configuration on a FAS8200 system  configured using VLAN 80 and 81 and you want to refresh to an AFF A900 system. AFF A900 does not support VLAN 80 and 81. In this scenario, you change the current RCF file to a VLAN that is supported, such as 110 and 120. Follow the steps in <<upgrade_rcf_expand_ip>> to upgrade the RCF, and use the new VLAN IDs. 
+
