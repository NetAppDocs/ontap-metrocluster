...
In ONTAP 9.13.1 and later, you can temporarily expand an eight-node MetroCluster configuration to upgrade the controllers and storage. Refer to link:https://docs.netapp.com/us-en/ontap-metrocluster/upgrade/task_refresh_4n_mcc_ip.html[Refreshing a four-node or an eight-node MetroCluster IP configuration (ONTAP 9.8 and later)] for more information.
...
* This procedure describes the steps required to add one four-node DR group to an existing MetroCluster IP configuration. If you are refreshing an eight-node configuration and need to temporarily add two DR groups, you must repeat the entire procedure for each DR group, adding one at a time.
