...
[.lead]
Starting with ONTAP 9.9.1, you can add four new nodes to the MetroCluster IP configuration as a second DR group. This creates an eight-node MetroCluster configuration.

* You must complete the initial steps in xref:task_refresh_4n_mcc_ip.adoc[Refreshing a four-node MetroCluster IP configuration (ONTAP 9.8 and later)].
+
After completing this procedure, return to xref:task_refresh_4n_mcc_ip.adoc[Refreshing a four-node MetroCluster IP configuration (ONTAP 9.8 and later)] to complete the refresh of the configuration.
