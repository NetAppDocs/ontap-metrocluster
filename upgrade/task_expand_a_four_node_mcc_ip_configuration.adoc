...
. On each site, add the new intercluster LIFs configured to the peer relationship.
+
From cluster_A, run the following command:
+
`cluster peer modify -cluster cluster_B -peer-addrs`
+
From cluster_B, run the following command:
+
`cluster peer modify -cluster cluster_A -peer-addrs`
