...
+
For example, you have a four-node MetroCluster configuration on a FAS8200 system  configured using VLAN 80 and 81 and you want to refresh to an AFF A900 system. AFF A900 does not support VLAN 80 and 81. In this scenario, you change the current RCF file to a VLAN that is supported, such as 110 and 120. Follow the steps in <<Modify the VLAN on MetroCluster IP interfaces>>. 

* You *must* upgrade one network at a time, in the following order: Switch-A1, Switch-B1, Switch-A2, Switch-B2.

* When you change the VLAN in a shared layer 2 or layer 3 network, the ISL configurations in your network also need to be updated to the allow the new VLANs.
