...
. If the MetroCluster nodes are using encryption for volumes or aggregates, or if the system has locked SED drives, copy information about the keys and passphrases.
...
For additional information, see https://docs.netapp.com/ontap-9/topic/com.netapp.doc.pow-nve/GUID-1677AE0A-FEF7-45FA-8616-885AA3283BCF.html[Back up onboard key management information manually].
 ...
 .. If external key management (KMIP) is configured, issue the following commands:
+
`security key-manager external show -instance`
+
`security key-manager key query`
... For external key management, copy the key management configuration file to a secure location. 
