...
. If the MetroCluster nodes are configured with an external key manager for the admin SVM (cluster), copy information about the keys. If the Onboard Key Manager is configured, copy the backup information and passphrase to a secure location.
+ 
[role="tabbed-block"]
====
.Onboard Key Manager
...
.. Obtain the primary key server port numbers and addresses for the admin SVM (cluster) by running the following command:
+
