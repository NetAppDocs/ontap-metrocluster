...
. If the MetroCluster nodes are configured with an external key manager for the admin SVM (cluster), copy the information needed to retrieve keys from a key server to a secure location. If the Onboard Key Manager is configured, copy the backup information and passphrase to a secure location.
+ 
[role="tabbed-block"]
====
.Onboard Key Manager
...
+
You'll also need the most up-to-date passphrase later in the upgrade procedure. 
+
...
.. Obtain the primary key server addresses and port numbers for the admin SVM (cluster) by running the following command:
+
NOTE: The admin SVM can have up to four primary key servers.
+
...
* The primary key server addresses and port numbers for the admin SVM
* Keystore UUID for the admin SVM
