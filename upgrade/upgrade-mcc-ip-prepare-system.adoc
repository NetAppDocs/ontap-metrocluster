...
. If the MetroCluster nodes are configured with a key manager for the admin SVM (cluster), copy information about the keys and passphrases.
+ 
[role="tabbed-block"]
====
.Onboard Key Manager
...
+
You'll need the passphrase later in the upgrade procedure.
+
...
--
Perform the following steps to gather the external key management information that you'll need later in the procedure to restore the configuration.

. Obtain the primary key server port and address for the admin SVM by running the following command:
+
`security key-manager external show -instance`
+
----
cluster_B::> security key-manager external show -instance
Vserver: datavs
...
.. Display information about the admin SVM:
+
...
.. Get the keystore UUID of the admin SVM:
+
...
. Save the information you gathered in a secure location for use later in the proecedure. You should now have gathered the following information:
* Primary key server address and port for the admin SVM
