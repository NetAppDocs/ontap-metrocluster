...
To enable networking to resume cleanly on the new controllers, you verify the correct LIF placement, and  remove the networking configuration of the old controllers.
...
. [[upgrades_assisted_without_matching_ports]] Modify broadcast domains to remove VLAN and physical ports that need to be deleted:
+
