...
** For information on what platform upgrade combinations are supported, review the MetroCluster IP upgrade table in  link:concept_choosing_controller_upgrade_mcc.html#supported-metrocluster-ip-controller-upgrades[Choose a controller upgrade procedure].
...
* The systems must be running the same ONTAP version. 
...
+
Use the steps in link:task_upgrade_A700_to_A900_in_a_four_node_mcc_ip_us_switchover_and_switchback.html#configure-the-new-controllers[Set the MetroCluster IP bootarg variables].
+
NOTE: Failure to do so will result in missing disks after you reassign the root aggregate disks to the new controller module.
