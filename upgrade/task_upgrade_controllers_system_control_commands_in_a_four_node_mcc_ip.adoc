...
== Replace the old controllers and booting up the new controllers
...
External key management
a|
`security key-manager external restore -vserver <svm-name> -node <node-name> -key-server _host_name\|IP_address:port_ -key-id key_id -key-tag key_tag <node-name>`
