...
For more information on supported platform upgrade combinations, review the MetroCluster IP upgrade table in  link:concept_choosing_controller_upgrade_mcc.html#supported-metrocluster-ip-controller-upgrades[Choose a controller upgrade procedure].
...
* The MetroCluster IP switches (switch type, vendor, and model) and the firmware version must be supported on the existing and new controllers you are upgrading. 
+
...
You must rack and install the controllers, perform required setup in Maintenance mode, boot the controllers, and verify the LIF configuration on the controllers. 
...
+
Installing the new controllers separately is not required for upgrades of integrated systems with disks and controllers in the same chassis, for example, from an AFF A800 system to an AFF A90 system. The new controller modules and I/O cards must be swapped after powering off the old controllers. 
+
The following example image is for representation only. 
+
image::../media/a90_a70_pcm_swap.png[]
