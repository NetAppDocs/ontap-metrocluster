...
. Modify the intercluster LIFs on the old controllers to use a different home port than the ports used for HA interconnect or MetroCluster IP DR on the new controllers.
+
NOTE: This step must be performed for a successful upgrade.
+
The intercluster LIFs on the old controllers must use a different home port than the ports used for HA interconnect or MetroCluster IP DR on the new controllers. For example, when you upgrade to AFF A90 controllers, the HA interconnect ports are e1a and e7a, and the MetroCluster IP DR ports are e2b and e3b. The intercluster LIFs on the old controllers must be moved if they are hosted on ports e1a,e7a, e2b, or e3b.
+
...
+
Take one of the following actions depending on whether the intercluster LIFs on the old controllers use the same ports as the ports used for HA interconnect or MetroCluster IP DR on the new controllers.
+
...
+
`network interface modify -vserver <vserver> -lif <intercluster_lif> -home-port <port-not-used-for-ha-interconnect-or-mcc-ip-dr-on-new-nodes>`
+
.. Verify that all intercluster LIFs are now on their home ports:
+
`network interface show -role intercluster -is-home  false`
+
The command output should be empty, indicating all that all intercluster LIFs are on their respective home ports. 
