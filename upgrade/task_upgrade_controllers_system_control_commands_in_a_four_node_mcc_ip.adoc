...
* You need the UUIDs and system IDs identified earlier in the upgrade procedure in <<gather_info_so_sb,Gather information before the upgrade>>.
...
* The MetroCluster bootarg values you set depend on whether your new system uses cluster HA ports or MetroCluster HA ports. 
+
The systems listed in the following table use *MetroCluster HA ports*. All other systems use *cluster HA ports*. 
+
[cols=2*,options="header""]
|===
h| AFF and ASA systems that use MetroCluster HA ports h| FAS systems that use MetroCluster HA ports
a| 
* AFF A150, ASA A150 
* AFF A220
* AFF C250, ASA C250
...
+
The steps you follow depends on the new platform model.
+
[role="tabbed-block"]
====
.Upgrades to systems that use cluster HA ports
--
...
-- 
.Upgrades to systems that use MetroCluster HA ports
--
.. Set the following bootargs:
+
