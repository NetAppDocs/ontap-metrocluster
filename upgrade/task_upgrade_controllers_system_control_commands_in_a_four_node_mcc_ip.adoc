...
. Perform a giveback when the node comes back to waiting for giveback prompt. 
 ...
 ** `set diagnostic; volume efficiency show -fields compression, compression-algorithm, storage-efficiency-mode -volume  *` 
 ** `node show -node  * -fields is-capacity-optimized` 
 ** `node show -node  * -fields  is-cbse-default` 
 ** `node show -node * -fields  is-qat-supported` 
 ** `vol show -fields type` 
 ** `run local aggr options data_aggr` 
 ** `run * version` 
...
+
Installing the new controllers separately does not apply when you upgrade integrated systems with disks and controllers in the same chassis to new controllers by swapping the PCMs and I/O cards, for example, from an AFF A900 to an AFF A90. 
+
PCMs of the new controllers and I/O cards must be swapped after powering off the old controllers. 
+
<image_needed>
+ 
