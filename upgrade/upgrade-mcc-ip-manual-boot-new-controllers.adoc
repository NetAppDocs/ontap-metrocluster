...
*Note:* You'll need the port numbers and addresses of the primary keys for the admin SVM and the admin SVM UUID that you gathered earlier in the procedure.
|===
...
*Note:* You'll need the port number and address of the primary key server for the admin SVM and the admin SVM UUID to restore the key manager configuration.
|===
+
...
External key management
a|
`security key-manager external restore -vserver <SVM> -node <node_name> -key-server <host_name\|IP_address:port> -key-id key_id -key-tag key_tag`
