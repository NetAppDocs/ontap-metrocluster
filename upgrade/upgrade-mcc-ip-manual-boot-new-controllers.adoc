...
. Restore the key management configuration based on your encryption type. 
+
[role="tabbed-block"]
====
.Root volume encryption
--
If you are using root volume encryption, select the correct boot menu option for your key
...
.NSE or SED drives
--
If there are NetApp Storage Encryption (NSE) or SED drives on the system, select Option `10` for Onboard Key Management. 

Follow the prompts and provide the required inputs to recover and restore the key manager configuration.
