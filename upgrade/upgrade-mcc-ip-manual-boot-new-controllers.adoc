...
* The admin SVM (cluster) UUID
* The keystore UUID for the admin SVM

*Note:* Although the admin SVM can have up to four primary key servers, only information for one primary key server can be specified during the recovery process. If there is a username and password associated with the chosen key server, you'll also need to provide that information. 
|===
...
|===
+
Depending on the key manager type, perform the recovery procedure by selecting option "`10`" or option "`11`", followed by option `6` at the first boot menu prompt. To boot the nodes completely, you might need to repeat the recovery procedure continued by option "`1`" (normal boot).
...
. If encryption is enabled on the admin SVM, restore the keys using the correct command for your key management configuration.
+
