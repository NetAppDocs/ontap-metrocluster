...
*Note:* Although the admin SVM can have up to four primary key servers. Only the port number and address for one primary key server can be specified during the recovery process. If there is a username and password associated with the chosen key server, you'll also need to provide that information. 
|===
