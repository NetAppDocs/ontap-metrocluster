...
+
Use the steps in link:upgrade-mcc-ip-system-controller-replace-apply-rcf-set-bootarg.html#set-the-metrocluster-ip-bootarg-variables[Set the MetroCluster IP bootarg variables].

. Verify that the partner-sysid is correct:
+
`printenv partner-sysid`
+
If the partner-sysid is not correct, set it:
...
.. After the intercluster LIFs are up, check the cluster peer status and reestablish cluster peering as needed.
+
...
External key management
a|
`security key-manager external restore -vserver <svm-name> -node <node-name> -key-server <host_name\|IP_address:port> -key-id <key_id> -key-tag <key_tag>`
