---
permalink: upgrade/task_refreshing_a_four_node_metrocluster_configuration_using_volume_move.html
sidebar: sidebar
keywords: 
summary: ''
---
...
. To prevent automatic support case generation, send an Autosupport message to indicate the upgrade is underway.
 .. Issue the following command: `system node autosupport invoke -node * -type all -message "MAINT=10h Upgrading old-model to new-model"`
+
...
+
link:task_refreshing_a_four_node_metrocluster_configuration_using_volume_move.md#[Adding new nodes to the MetroCluster IP configuration]
+
...
+
Perform the steps in https://docs.netapp.com/ontap-9/topic/com.netapp.doc.hw-metrocluster-service/task_moving_a_metadata_volume_in_metrocluster_configurations.html[Moving a metadata volume in MetroCluster configurations].
...
* You must complete the initial steps in link:task_refreshing_a_four_node_metrocluster_configuration_using_volume_move.md#[Refreshing a four-node MetroCluster IP configuration (ONTAP 9.8 and later)].
+
After completing this procedure, return to link:task_refreshing_a_four_node_metrocluster_configuration_using_volume_move.md#[Refreshing a four-node MetroCluster IP configuration (ONTAP 9.8 and later)] to complete the refresh of the configuration.
...
. Cable the new MetroCluster IP nodes to the IP switches as shown in the _MetroCluster Installation and Configuration Guide._
+
...
. Configure the MetroCluster IP nodes using the following sections of the _MetroCluster Installation and Configuration Guide._
 .. http://docs.netapp.com/ontap-9/topic/com.netapp.doc.dot-mcc-inst-cnfg-ip/GUID-C2ED215D-DDDD-43E7-9F50-15D37D95F505.html[Gathering required information]
 .. http://docs.netapp.com/ontap-9/topic/com.netapp.doc.dot-mcc-inst-cnfg-ip/GUID-BB31AA18-EC61-486F-A8F8-070B47793C91.html[Clearing the configuration on a controller module]
 .. http://docs.netapp.com/ontap-9/topic/com.netapp.doc.dot-mcc-inst-cnfg-ip/GUID-A97BA5C2-E15D-4BD0-BA74-4FC6F969845B.html[Verifying the ha-config state of components]
 .. http://docs.netapp.com/ontap-9/topic/com.netapp.doc.dot-mcc-inst-cnfg-ip/GUID-862DA05B-DB68-4E78-AE23-48D87148B2A3.html[Manually assigning drives for pool 0 (ONTAP 9.4 and later)]
...
+
http://docs.netapp.com/ontap-9/topic/com.netapp.doc.dot-mcc-inst-cnfg-ip/task_configuring_intercluster_lifs_to_use_dedicated_intercluster_ports.html[Configuring intercluster LIFs on dedicated ports]
+
http://docs.netapp.com/ontap-9/topic/com.netapp.doc.dot-mcc-inst-cnfg-ip/task_configuring_intercluster_lifs_to_share_data_ports.html[Configuring intercluster LIFs on shared data ports]
...
. Create the DR group for the MetroCluster IP nodes: `metrocluster configuration-settings dr-group create -partner-cluster`
+
For more information on the MetroCluster configuration settings and connections, see the _MetroCluster IP Installation and Configuration Guide_.
+
https://docs.netapp.com/ontap-9/topic/com.netapp.doc.dot-mcc-inst-cnfg-ip/GUID-DF601A9E-AA7A-468E-8BAC-5A169650F095.html[Considerations for MetroCluster IP configuration]
+
https://docs.netapp.com/ontap-9/topic/com.netapp.doc.dot-mcc-inst-cnfg-ip/GUID-D32C3191-B17F-4216-9FB0-DB062944B4DE.html[Creating the DR group]
+
----
...
. Verify that the DR group was created. `metrocluster configuration-settings dr-group show`
+
----
...
. Mirror the root aggregates: `storage aggregate mirror -aggregate aggr0_node_A_1-new`
+
...
. Refresh the MetroCluster configuration:
 ...
 .. Refresh the MetroCluster configuration on one of the new nodes: `metrocluster configure`
+
...
----
 ...
 .. Set epsilon to false on the old node (node_A_1-old): `cluster modify -node old-node -epsilon false`
 .. Set epsilon to true on the new node (node_A_3-new): `cluster modify -node new-node -epsilon true`
...
4 entries were displayed.
----
. Complete the remaining steps in link:task_refreshing_a_four_node_metrocluster_configuration_using_volume_move.md#[Refreshing a four-node MetroCluster IP configuration (ONTAP 9.8 and later)].
+
