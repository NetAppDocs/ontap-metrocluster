...
CAUTION: If SVM migration is in progress, you must wait until all migration processes are complete before you can initiate the controller upgrade or system refresh procedures. Do not initiate new SVM migrate operations during the upgrade or refresh process.
