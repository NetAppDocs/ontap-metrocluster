...
The following guidance is for a rare scenario where you need to add a newer platform model (platforms released in ONTAP 9.15.1 or later) to an existing MetroCluster configuration containing an older platform model (platforms released earlier than ONTAP 9.15.1). 

During the transition, refresh, and expansion procedures, you might add a new platform model to your MetroCluster configuration. At any stage during these procedures, if your existing MetroCluster configuration contains a platform that uses *shared cluster/HA ports* (platforms released in ONTAP 9.15.1 or later), you cannot add a platform that uses *shared MetroCluster/HA ports* (platforms released earlier than ONTAP 9.15.1) to the configuration without upgrading your ONTAP version to ONTAP 9.15.1P11 or ONTAP 9.16.1P4 or later. 

[CAUTION] 
====
Adding a newer platform model that uses *shared cluster/HA ports* to a MetroCluster containing an older platform model that uses *shared/MetroCluster HA ports* is a rare scenario and most upgrade combinations are not affected. 
==== 
