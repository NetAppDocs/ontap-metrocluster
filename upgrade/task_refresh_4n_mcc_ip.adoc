...
[.lead]
You can use this procedure to upgrade the controllers and storage in a four-node or an eight-node configuration.
...
. Modify the cluster's peer IP address of the transitioned IP nodes for each cluster:
