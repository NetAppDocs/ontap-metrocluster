...
[CAUTION] 
====
Adding an older platform model that uses *shared/MetroCluster HA ports* to a MetroCluster containing an newer platform model that uses *shared cluster/HA ports* is an uncommon scenario and most upgrade combinations are not affected. 
==== 
