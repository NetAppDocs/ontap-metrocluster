...
[CAUTION] 
====
Adding a newer platform model that uses *shared cluster/HA ports* to a MetroCluster containing an older platform model using *shared/MetroCluster HA ports* is a rare scenario and most upgrade combinations are not affected. 
