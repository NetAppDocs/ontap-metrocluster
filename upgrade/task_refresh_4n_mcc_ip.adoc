...
* The table shows all platform models that use *shared MetroCluster/HA ports* and *shared cluster/HA ports*. Not all combinations are supported by this procedure. Refer to the link:concept_choosing_tech_refresh_mcc.html#supported-metrocluster-ip-tech-refresh-combinations[supported platform combinations] to verify if your refresh combination is supported.
==== 
...
+
image::../media/mcc_dr_group_a.png["MetroCluster IP four-node configuration before expansion"]
+
The eight-node configuration appears as shown in the following image:
+
image::../media/mcc_dr_groups_8_node.gif["MetroCluster IP configuration with eight nodes after expansion"]
...
+
The following example specifies a 10 hour maintenance window. Allow additional time depending on your plan.
+
...
+
image::../media/mcc_dr_group_b.png["MetroCluster configuration after expansion and CRS volume migration"]
+
.Temporary twelve-node configuration
+
image::../media/mcc_dr_group_c4.png["Temporary twelve-node MetroCluster configuration"]
...
+
. If you need to refresh both DR groups in an eight-node configuration, repeat the entire procedure for each DR group.
+
...
image::../media/mcc_dr_group_d.png["MetroCluster configuration after removing old DR group"]
+
.Eight-node configuration
+
image::../media/mcc_dr_group_c5.png["Final eight-node MetroCluster configuration"]
...
Tiebreaker
a|
link:../tiebreaker/concept_configuring_the_tiebreaker_software.html#add-mcc-config-tb[Adding MetroCluster configurations] in the _MetroCluster Tiebreaker Installation and Configuration_.
a|
Mediator
a|
