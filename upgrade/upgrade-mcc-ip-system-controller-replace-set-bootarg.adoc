...
CAUTION: If the upgrade you are performing is listed in <<upgrade_paths_bootarg_assisted,this table>>, you *must* set the `setenv bootarg.siw.interop_enabled 1` bootarg on the existing system before performing the upgrade. For all other upgrades, you can skip this task and go directly to link:upgrade-mcc-ip-system-controller-replace-prechecks.html[Prepare the system for upgrade].
