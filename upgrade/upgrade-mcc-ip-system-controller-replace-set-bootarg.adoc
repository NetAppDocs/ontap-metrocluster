...
This task is *only* required when you upgrade to an AFF A30, AFF A20, or AFF C30 system.

CAUTION: If you are upgrading to an AFF A30, AFF A20, or AFF C30 system, you *must* set the `setenv bootarg.siw.interop_enabled 1` bootarg on the existing system before performing the upgrade. For all other upgrades, you can skip this task and go directly to link:upgrade-mcc-ip-system-controller-replace-prechecks.html[Prepare the system for upgrade].
