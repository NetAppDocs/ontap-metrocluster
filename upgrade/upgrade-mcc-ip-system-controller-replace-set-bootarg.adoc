...
CAUTION: If you are upgrading to any of the systems listed, you *must* set a required bootarg on the existing system before performing the upgrade. For all other upgrades, you can skip this task and go directly to link:link:upgrade-mcc-ip-system-controller-replace-prechecks.html[Prepare the system for upgrade][Prepare the system for upgrade].
...
. Set the required bootarg for your upgrade combination. You should have already determined the bootarg that you need to set by using the table in <<upgrade_paths_bootarg_assisted,determine which bootarg you need to set>>. 
+
[role="tabbed-block"]
====
