...
[.lead]
Move the MetroCluster FC configuration's cluster interfaces to the IP switches. The steps you follow depend on whether you are using the existing IP switches or you are using new IP switches.
...
== Verify that the cluster connections are moved and the cluster is healthy
