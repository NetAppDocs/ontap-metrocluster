...
a| Requires adding one MetroCluster IP DR group. You need to check the overall health of the cluster and repeat the steps for the second MetroCluster IP DR group.

a| Eight-node FC to four-node IP configuration
a| Follow this procedure as written.

a| Eight-node FC to eight-node IP configuration
a| Requires adding one MetroCluster IP DR group. You need to check the overall health of the cluster, move data, and unjoin the empty MetroCluste FC DR groups. You then add the second MetroCluster IP DR group. 
