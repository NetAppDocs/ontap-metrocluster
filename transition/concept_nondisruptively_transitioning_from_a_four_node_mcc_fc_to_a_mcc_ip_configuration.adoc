...
When you are applying the RCF as part of this procedure, you can apply the four-node FC to eight-node IP RCF as part of this procedure before you add the second DR group. You do not have to apply the four-node FC to four-node IP RCF first.

a| Eight-node FC to four-node IP configuration
a| Follow this procedure as written.

a| Eight-node FC to eight-node IP configuration
a| Requires adding one MetroCluster IP DR group at a time. After you add the first MetroCluster IP DR group, you need to check the overall health of the cluster, move data to the newly added DR group, and unjoin the empty MetroCluster FC DR group. You then add the second MetroCluster IP DR group and continue the transition.  
