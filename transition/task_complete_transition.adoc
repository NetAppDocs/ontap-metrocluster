...
. If you have an eight-node configuration, you must repeat the entire procedure starting from link:concept_requirements_for_fc_to_ip_transition_mcc.html[Prepare for transition from a MetroCluster FC to a MetroCluster IP configuration] for each FC DR group.
