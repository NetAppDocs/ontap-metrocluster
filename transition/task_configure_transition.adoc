...
. If you are using the Onboard Key Manager, refer to link:https://docs.netapp.com/us-en/ontap/encryption-at-rest/enable-onboard-key-management-new-nodes-concept.html[Enable onboard key management in newly added nodes] to synchronize the key manager configuration for the new nodes.
