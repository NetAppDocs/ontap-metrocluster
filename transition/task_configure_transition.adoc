...
. If you are using the Onboard Key Manager, perform the following steps from the cluster where you've added a new node:
+
.. Synchronize the key manager configuration:
+
`security key-manager onboard sync`

.. Enter the Onboard Key Manager passphrase when prompted.
