...
+
https://docs.netapp.com/us-en/ontap/networking/create_a_lif.html[Creating network interfaces^]
+
...
12 entries were displayed.
----
. To avoid a `rescan-scsi-bus.sh utility not found` error, verify that the `sg3-utilis` package is installed on the client host.
...
. On the iSCSI host, verify that only paths from the MetroCluster IP nodes are visible: 
+
`sanlun lun show -p`
+
`multipath -ll -d`
