...
a| Nondisruptive
a| New storage shelves are required on new platforms. After the transition is completed, old controllers, shelves and disks are removed from the cluster.

a| link:concept_nondisruptively_transitioning_from_a_four_node_mcc_fc_to_a_mcc_ip_configuration.html[Link to procedure]

*Note:* This procedure supports the following FC to IP transitions:

*  From a four-node MetroCluster FC configuration to a four-node MetroCluster IP configuration 

* From an eight-node MetroCluster FC configuration to an eight-node MetroCluster IP configuration
a| Two node
a| Disruptive
