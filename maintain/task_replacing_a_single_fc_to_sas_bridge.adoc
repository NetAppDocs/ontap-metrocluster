---
permalink: maintain/task_replacing_a_single_fc_to_sas_bridge.html
sidebar: sidebar
keywords: 
summary: ''
---
...
link:reference_requirements_for_using_other_interfaces_to_configure_and_manage_fibrebridge_bridges.md#[Requirements for using other interfaces to configure and manage FibreBridge bridges]
...
a|
Use the ONTAP CLI to retrieve the configuration information with the following commands: `storage bridge run-cli -name bridge-name -command "info"``storage bridge run-cli -name bridge-name -command "sasportlist"`
|===
Enter the command: `storage bridge run-cli -name bridge_A1 -command "info"`
+
----
info
...
----
+
Enter the command: `storage bridge run-cli -name bridge_A1 -command "sasportlist"`
+
----
...
Correct
a|
Repeat Step link:task_replacing_a_single_fc_to_sas_bridge.md#STEP_524EBC3334F54467B771D9E2FD2B2DDA[task_replacing_a_single_fc_to_sas_bridge.md#STEP_524EBC3334F54467B771D9E2FD2B2DDA] for each remaining bridge.
a|
Not correct
a|

 .. Check for loose SAS cables or correct the SAS cabling by repeating Step link:task_replacing_a_single_fc_to_sas_bridge.md#STEP_CD84065D8F3B43F192919B0CD6FDC1A6[task_replacing_a_single_fc_to_sas_bridge.md#STEP_CD84065D8F3B43F192919B0CD6FDC1A6].
 .. Repeat Step link:task_replacing_a_single_fc_to_sas_bridge.md#STEP_524EBC3334F54467B771D9E2FD2B2DDA[task_replacing_a_single_fc_to_sas_bridge.md#STEP_524EBC3334F54467B771D9E2FD2B2DDA].
...
. From the system console of both controller modules, verify that all of the controller modules have access through the new bridge to the disk shelves (that is, that the system is cabled for Multipath HA): `run local sysconfig`
+
...
If using the CLI, you must run the following command: `set FCDataRate port-numberport-speed`
  ...
  *** If you are using the second port, you must repeat the previous substeps for the FC2 port.
  *** If you are not using the second port, then you must disable the unused port: `FCPortDisable port-number`
...
----
 .. Disable the unused SAS ports: `SASPortDisable sas-port`
+
...
IMPORTANT: If you are hot-swapping both FibreBridge 6500N bridges in a pair, you must use the link:task_fb_consolidate_replacing_a_pair_of_fibrebridge_6500n_bridges_with_7500n_bridges.md#[Consolidate Multiple Storage Stacks] procedure for zoning instructions. By replacing both FibreBridge 6500N bridges on the bridge, you can take advantage of the additional ports on the FibreBridge 7600N or 7500N bridge.
...
The ATTO FibreBridge status page appears with a menu to the left.
 ...
 .. Click *Advanced* in the menu.
 .. Enter the following command and then click *Submit* to see the list of disks visible to the bridge: `sastargets`

a|
Serial port connection
a|
Display the list of disks visible to the bridge: `sastargets`
|===
