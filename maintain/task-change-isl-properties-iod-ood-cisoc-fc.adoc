...
---
= Change ISL properties or the IOD/OOD configuration on a Cisco switch
:icons: font
:imagesdir: ../media/
...
+
Verify that these ports are listed in the cabling documentation the for the switch model you are adding them to.

. Configure the ISL ports by following the steps in the link:/install-fc/task_cable_the_isl_between_the_mcc_sites_fabric_config.html[Cabling the ISLs between MetroCluster sites].
...
+
Verify that the ISL ports for your switch are listed in the _Fabric-attached MetroCluster Installation and Configuration Guide_.
...
NOTE: The standard in-order delivery (IOD) settings are recommended. You should configure out-of-order delivery of frames (OOD) only if necessary.
+
