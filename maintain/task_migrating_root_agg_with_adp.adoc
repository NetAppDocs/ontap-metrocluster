...
* You can only migrate one root aggregate at a time. Do not attempt to migrate two root aggregates at the same time. 

.Steps 

. [[step_1, verify the health of the configuration]]Check the health of the configuration.
...
. Confirm that takeover is possible: 
+
`storage failover show`
+
----
...
. Repeat the step to <<step_1,verify the health of the configuration>>. 
