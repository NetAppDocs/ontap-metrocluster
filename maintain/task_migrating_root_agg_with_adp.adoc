...
---
= Migrate an existing root aggregate with ADP enabled 
:icons: font
:imagesdir: ../media/

[lead]
You can nondisruptively migrate an existing root aggregate with Advanced Disk Partitioning (ADP) enabled to a larger root aggregate with ADP enabled. You can use this procedure if the root aggregate is running out of space or if you want to switch from using low capacity SSDs to large capacity SSDs.
...
* During this procedure, do not perform any hardware upgrades or any other operations that could bring down a node. 
...
+
----
siteA::*> node run local aggr status -r <root_agg_name>
...
+
Before you assign the disks, confirm that the recommended number of spare drives are assigned to each node. These drives are partitioned before the root aggregate is placed on them. Use the following command to assign the disks: 
+
...
+
`cluster-01*> disk partition -n 3 -i 3 -b <root_partition_size> <disk_id>` 
+
