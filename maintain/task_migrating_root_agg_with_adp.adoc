...
+
Run the `job show -id 51 -instance` on the second site if you want to monitor the migration progress. The migration process might take several minutes to complete. During migration, the node reboots several times and you might see errors on the other nodes, you can safely ignore these errors and wait for the migration process to finish.  
