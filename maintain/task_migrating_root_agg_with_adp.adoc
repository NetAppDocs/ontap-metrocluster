...
---
= Migrate a root aggregate with ADP
:icons: font
:imagesdir: ../media/

[lead]
You can migrate a root aggregate with ADP to a larger root aggregate with ADP. 
...
+
`cluster::> storage disk assign -disklist <disklist> -owner <cluster-01> -pool 0`
...
+
`cluster-01*> disk partition -n 3 -i 3 -b <root_partition_size> <disk_name>` 
+
. Assign the partitions:
+
* Assign the P3 partition to the same node that owns the container disk:
+
`storage disk assign -disk <disk_name> -root true -pool 0 -owner cluster-01`

* Assign the P1 partition to the system with the lower system ID number in the HA pair:
+
`storage disk assign -disk <disk_name> -data1 true -pool 0 -owner cluster-01`
