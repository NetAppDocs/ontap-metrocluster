...
NOTE: If you are using a Cisco 9336C-FX2 switch in a shared configuration connecting NS224 shelves, changing the ISLs might require a new reference configuration file (RCF). You do not require a new RCF if your current and new ISL speeds are 40 Gbps and 100 Gbps. All other changes to ISL speed require a new RCF. For example, changing the ISL speed from 40 Gbps to 100 Gbps does not require a new RCF, but changing the ISL speed from 10 Gbps to 40 Gbps requires a new RCF.
...
+
NOTE: If you are using Cisco 9336C-FX2 switches in a shared configuration connecting NS224 shelves, and you need to upgrade the RCF and apply the new configuration for the new ISL ports, follow the steps to link:task_upgrade_rcf_files_on_mcc_ip_switches.html[upgrade the RCFs on MetroCluster IP switches.]
+
[cols="30,70"]
|===
