...
---
= Upgrade or downgrade the firmware on a Brocade MetroCluster FC switch
:icons: font
:imagesdir: ../media/
...
Review the following information carefully before using this procedure: 


* Beginning with Fabric OS 10.x, Brocade supports only SHA-512 for SNMP authentication. ONTAP currently does not support SHA-512, with support limited to SHA-256.
+
If you upgrade your MetroCluster FC switch firmware to Fabric OS 10.x, SNMP monitoring visibility from ONTAP will be lost. This leads to increased risk when troubleshooting and the loss of proactive alerting. 
+
If you upgrade to or downgrade from Fabric OS 10.x, you must perform the mandatory tasks outlined in link:https://techdocs.broadcom.com/us/en/fibre-channel-networking/fabric-os/fabric-os-software-upgrade/10-0-x/Upgrading-and-Downgrading-Firmware/v24512876.html[Brocade Fabric OS Software Upgrade Guide, 10.0.x - Upgrade or Downgrade Prerequisites^].
+
CAUTION: You must carefully consider these limitations before you proceed to upgrade to or downgrade from Fabric OS 10.x or later. 


* Beginning with Fabric OS 9.0.1, SNMPv2 is not supported on Brocade switches. If you upgrade to Fabric OS 9.0.1 or later, you must use SNMPv3 for health monitoring. For more information, see link:../install-fc/concept_configure_the_mcc_software_in_ontap.html#snmpv3-mcc-fc[Configuring SNMPv3 in a MetroCluster configuration].

* If you are upgrading to Fabric OS 9.2.x or later, you must have a Brocade TruFOS certificate installed. Refer to link:https://techdocs.broadcom.com/us/en/fibre-channel-networking/fabric-os/fabric-os-software-upgrade/9-2-x/Obtaining-Firmware/download-prerequisites-new/brocade-trufos-certificates.html[Brocade Fabric OS Software Upgrade Guide, 9.2.x^] for more information.

.About this task

Confirm that you have checked and performed the following tasks for your configuration: 

* Verify that your new firmware version is compatible with your solution. See the link:https://hwu.netapp.com/[Hardware Universe^] for more information.
