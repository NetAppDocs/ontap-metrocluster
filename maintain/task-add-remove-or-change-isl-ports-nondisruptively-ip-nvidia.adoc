---
permalink: maintain/task-add-remove-or-change-isl-ports-nondisruptively-ip-nvidia.html
sidebar: sidebar
...
---
= Add, remove, or change ISL ports nondisruptively on NVIDIA IP switches
:icons: font
:imagesdir: ../media/ 

[.lead]
You might need to add, remove, or change ISL ports on a NVIDIA IP switch. You can convert dedicated ISL ports to shared ISL ports, or change the speed of ISL ports on a NVIDIA IP switch. ISLs traveling greater distances might require lower speeds to improve quality. If you are adding or upgrading hardware, you might need to increase the speed of ISL ports to take advantage of system upgrades and add ISLs to a NVIDIA IP switch.
