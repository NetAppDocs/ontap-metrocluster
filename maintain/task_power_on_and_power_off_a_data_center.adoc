...
image::power-on-off-data-center.gif["Workflow for powering off Cluster_B in MetroCluster configuration for maintenance"]
...
.. Verify that the local and remote storage is visible from the node in maintenance mode:
+
`disk show -v`
