...
+
The steps you follow depend on whether credentials are required for the URI:
+
[role="tabbed-block"]
====
...
Enter the password when prompted, as shown in the following example:

----
cluster_A> storage bridge firmware update -bridge bridge_A_1a -uri http://192.168.132.97/firmware.ZBD -username abc
