...
CAUTION: If you need to perform a switch replacement on AFF A220, AFF A250 or FAS2750 platforms, refer to link:https://kb.netapp.com/on-prem/ontap/mc/MC-KBs/Cluster_and_HA_communication_issues_during_back-end_switch_replacement[Cluster and HA communication issues during back-end switch replacement^].
...
[NOTE]
====
Mixed link speeds are not supported on platforms that use an X60134A network interface card. To check if your platform is affected, refer to the https://hwu.netapp.com/[Hardware Universe^]. 
 
If you are replacing a switch that uses 25G ports with a new switch that uses 100G ports on an affected platform (for example replacing a BES-53248 switch with a Cisco 9336C switch on an AFF A50 platform), refer to link:https://kb.netapp.com/on-prem/ontap/mc/MC-KBs/100G_link_not_coming_online_during_MetroCluster_back-end_switch_replacement?caseId=2010523490[100G link not coming online during MetroCluster back-end switch replacement^]. 
====
...
+
image::../media/mcc_ip_hardware_architecture_ip_interconnect.png["MetroCluster IP hardware architecture showing inter-switch links and switch types"]
+
...
image:../media/mcc_port_usage_workflow.png["Workflow for MetroCluster IP port usage and cabling changes during switch replacement"]
...
+
NOTE: You must update the switches in the following order: Switch_A_1, Switch_B_1, Switch_A_2, Switch_B_2. You must repeat the steps to move the cluster LIFs on the second fabric (Switch_A_2, Switch_B_2) after you have configured the switches in the first fabric (Switch_A_1, Switch_B_1). Refer to link:../install-ip/task_install_and_cable_the_mcc_components.html[Learn about configuring the MetroCluster IP switches].
...
. Cable the switches. 
+
You can follow the steps in link:../install-ip/using_rcf_generator.html[Cable the MetroCluster IP switches]. 
+
...
+
If the new switch is the same, power up the new switch. If you are upgrading or downgrading the switches, then power up both switches. The configuration can operate with two different switches at each site until the second fabric is updated.

. Verify that the MetroCluster configuration is healthy by repeating <<all_step1,Step 1>>.
+
If you are upgrading or downgrading the switches in the first fabric, you might see some alerts related to local clustering.
+
NOTE: If you upgrade or downgrade the fabric, then repeat all of the steps for the second fabric.
