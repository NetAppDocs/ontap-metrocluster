...
* Your MetroCluster IP might already connect NS224 shelves directly or to dedicated storage switches.
====
...
+
In the <<Create a cabling worksheet>> example, the switches do not change. The MetroCluster ISLs are relocated and must be disconnected. All connections marked in green on the worksheet do not need to be disconnected.
