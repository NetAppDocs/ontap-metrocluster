...
[.lead]
You might need to verify that the MetroCluster components are healthy. Health check commands can be run from the CLI. In MetroCluster IP configurations running ONTAP 9.8 or later, you can also use ONTAP System Manager to monitor and troubleshoot health check alerts. 

Learn how to verify the health of the MetroCluster configuration depending on whether you are using the CLI or System Manager.
