...
[.lead]
You might need to upgrade a reference configuration file (RCF) on a Cisco IP switch. For example, an ONTAP upgrade or a switch firmware upgrade both require a new RCF.
...
** The https://mysupport.netapp.com/site/tools/tool-eula/rcffilegenerator[RcfFileGenerator] must be able to create the same RCF that is applied, with the same version and configuration (platforms, VLANs).
** The RCF that is applied was not provided by NetApp for a special configuration.
** The RCF was not altered before it was applied.
** The steps to reset the switch to factory defaults were followed before applying the current RCF.
** No changes were made to the switch(port) configuration after the RCF was applied.
+

If you do not meet these requirements, then you cannot use the CleanUpFiles that are created when generating the RCFs. However, you can leverage the function to create generic CleanUpFiles -- the cleanup using this method is derived from the output of `show running-config` and is best practice.
+
...
. link:../install-ip/switch-move-cluster-lifs.html[Move the cluster LIFs] before you begin the upgrade. 

. Determine the current RCF version, and which ports and VLANs are used: `IP_switch_A_1# show banner motd`
+
...
. [[Create-RCF-files-and-CleanUpFiles-or-create-generic-CleanUpFiles]] Create the RCFs and CleanUpFiles, or create generic CleanUpFiles for the current configuration.
+
...
[role="tabbed-block"]
====
.Option 1: Create the RCFs and CleanUpFiles
--
Use this procedure if the configuration meets the requirements.

.Steps

.. Use the RcfFileGenerator 1.4a (or later) to create the RCFs with the information that you retrieved in Step 1. The new version of the RcfFileGenerator creates an additional set of CleanUpFiles that you can use to revert some configuration and prepare the switch to apply a new RCF configuration.

.. Compare the banner motd with the RCFs that are currently applied. The platform types, switch type, port and VLAN usage must be the same.
+
NOTE: You must use the CleanUpFiles from the same version as the RCF and for the exact same configuration. Using any CleanUpFile will not work and might require a full reset of the switch.
+
NOTE: The ONTAP version the RCF is created for is not relevant. Only the RCF version is important.
+
NOTE: The RCF (even if it is the same version) might list fewer or more platforms. Make sure that your platform is listed.
...
. [[Create-the-new-RCF-files-for-the-new-configuration]] Create the 'new' RCFs for the new configuration.
Create these files in the same way that you created the files in the previous step, except choose the respective ONTAP and RCF version.
+

After completing this step you should have two sets of RCFs, each set consisting of twelve files.

. Download the files to the bootflash.


.. Download the CleanUpFiles that you created in <<Create-RCF-files-and-CleanUpFiles-or-create-generic-CleanUpFiles, Create the RCFs and CleanUpFiles, or create generic CleanUpFiles for the current configuration>>
+
NOTE: This CleanUpFile is for the current RCF that is applied and *NOT* for the new RCF that you want to upgrade to.
+

Example CleanUpFile for Switch-A1: `Cleanup_NX9336_v1.81_Switch-A1.txt`

.. Download the 'new' RCFs that you created in <<Create-the-new-RCF-files-for-the-new-configuration, Create the 'new' RCFs for the new configuration.>>
+
Example RCF for Switch-A1: `NX9336_v1.90_Switch-A1.txt`
+

.. Download the CleanUpFiles that you created in <<Create-the-new-RCF-files-for-the-new-configuration, Create the 'new' RCFs for the new configuration.>> This step is optional -- you can use the file in future to update the switch configuration. It matches the currently applied configuration.
+
...
+
If you find that any of the above items are configured, you might not be able to apply a new RCF configuration. However, you can revert to the previous configuration by reloading the switch *without* saving the running configuration to the startup configuration. The switch will come up with the previous configuration.

. Apply the RCF and verify that the ports are online.
.. Apply the RCF.
+
----
...
. Modify all cluster LIFs to enable auto-revert:
