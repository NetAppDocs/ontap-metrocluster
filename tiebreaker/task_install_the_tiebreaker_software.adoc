...
The following provides some guidelines on how to secure and harden the host OS and the database.

==== Secure the host

The following gives some guidelines to make the host on which MetroCluster Tiebreaker is installed on.

** Limit access of the ‘root’ user
*  You can use users that are able to elevate to root access.
* You can use users that are not able to elevate to root access to administer MetroCluster Tiebreaker.
** Use disk encryption
* You can enable disk encryption. This can be FullDiskEncryption (hardware) or encryption provided by the HostOS (software)
...
Service that may be enabled by default and may be disabled are
* HTTP/HTTPS server
* FTP server
* Telnet and RSH
...
+
NOTE: To update MetroCluster Tiebreaker you need to copy the installation file to host. You can copy this using SCP or HTTP(S) to the host.
+
NOTE: you must leave either the serial console access (if supported) or at least one protocol enabled to administer the host on distance. If you disable all protocols, then you require physical access to the host for administration.
...
.	Ability to create a database and tables
.	Ability to create a database user and set the password
.	Ability to associate the database user with the database and tables and assign access rights.
+
...
* Enable FIPS compliancy for MariaDB
** MetroCluster Tiebreaker can connect to a database that is in FIPS mode [**Tom --- we must check this**]


https://mariadb.com/docs/security/data-at-rest-encryption/es/
...
* Only one user can run metroCluster Tiebreaker as a non-root user (**Confirm**).
