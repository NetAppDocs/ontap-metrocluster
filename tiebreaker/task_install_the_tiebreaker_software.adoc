...
. Create a non-root user with sudo privileges to access sufficient administrative privileges.
. Log in to the host as the non-root user.

. <<install-tiebreaker, Install the MetroCluster Tiebreaker software>>  (installation requires the ability to access sudo privileges).
