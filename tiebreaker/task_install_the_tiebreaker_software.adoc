...
==== NEW -- Secure your host and database installation (BURT 1496659) 

==== NEW -- Install Tiebreaker as a non-root user (BURT 1500383)

You can install MetroCluster Tiebreaker as a non-root user. A non-root user can have sufficient privileges to perform the Tiebreaker installation, create tables, users, and set the user password etc.

 NOTE: Only one user can run metroCluster Tiebreaker as a non-root user.
 NOTE: ONTAP can be in FIPS-compliant mode.
 NOTE: System requirements

. Create a non-root user that can sudo to the host.
. Log in to the host as the non-root user.

. <<install-tiebreaker, Install the MetroCluster Tiebreaker software>>  (installation requires the ability to sudo to the host).

. Run the MetroCluster Tiebreaker CLI as the non-root user.
