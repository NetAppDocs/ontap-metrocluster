...
//FIPS might be supported with TB 1.5 -- check
...
==== NEW section placeholder: ** Secure your host and database installation** (BURT 1496659 -- we need 'hardening' guidelines) 

==== NEW section placeholder: install Tiebreaker as a non-root user, a non-root user that is able to 'sudo' on the host (BURT 1500383)
- optional: a non-root user in the database that has sufficient privileges to perform the Tiebreaker install (create tables, users, set user password etc)

  

 we need to document how to install and run MCCTB as non-root user

 NOTE: Only one user can run it etc?

. create non-root user

. install MCCTB (install requires sudo)

. run MCCTB cli as non root user

We may also want to provide information about ONTAP:
- can be in FIPS compliant mode
- user requirements
- etc
...
. Install Tiebreaker as a non-root user.
.. Create a user that can sudo to advanced privilege mode.
.. Create another administrative user in the database. 
+
NOTE: You can use root to install Tiebreaker, but this is not optimum. See "Securing your host and database" for more information.
