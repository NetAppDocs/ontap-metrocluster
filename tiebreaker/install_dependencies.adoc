...
+
NOTE: For securing the communication with Vault, you should use TLS.
+
----
...
+
You must run the `vault operator unseal` command for each of the three keys as shown in the following examples:
+
.. Unseal vault using key 1:
+
----
...
+
NOTE: When you are prompted for the root password, you can leave it empty to continue to configure the security and password settings. 
+
`[root@mcctb ~]# mysql_secure_installation`
+
----
