...
. Install the appropriate JRE version for the version of Tiebreaker you are installing:
+
[source,cli]
...
+
Run the following command to create the `config.hcl` file:
+
----
...
----
.. Run the following command to create the vault service file.
+
----
...
[[install-mariadb]]
=== Installing MariaDB server on Red Hat Enterprise Linux 8 or CentOS
...
+
NOTE: When you are prompted for the root password, you leave it empty and press enter to continue to configure the security and password settings. 
+
`[root@mcctb ~]# mysql_secure_installation`
+
----
