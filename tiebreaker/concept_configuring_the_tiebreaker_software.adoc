...
* As part of this task, you perform a switchover simulation to check if you need to import a certificate. If the switchover simulation fails, you need to import certificates from ONTAP to the the key store in the Java Virtual Machine (JVM).

.Step

. Check if you need to import a certificate:
+
...
+
The command returns the following output if you need to import certificates to Tiebreaker:
+
----
