...
+
The `<certificate_file_name>` value specifies the file name of the certificate that you want to import. The `-alias <certificate>` value specifies the name that you want to store the certificate under after it is imported to the JVM.
+
...
+
The following example shows how to import a root certificate with the file name `root.crt` and an SSL server certificate with the name `ssl_cert.crt`:
+
----
