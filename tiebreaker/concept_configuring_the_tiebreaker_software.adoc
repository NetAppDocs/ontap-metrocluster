...
. In Tiebreaker 1.7 or later, run a switchover simulation to check if you need to import certificates. In Tiebreaker 1.6 or earlier, skip to <<tb_import_certificates, Step 2>> and import all of the certificates that you have in ONTAP. 
+
...
. In Tiebreaker 1.7 or later, perform the switchover simulation checks again:
+
.. Check the Tiebreaker monitoring status:
`monitor show -status`
+
