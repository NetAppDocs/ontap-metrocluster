...
* You must have installed OpenJDK by using the `yum install java-x.x.x-openjdk` command. For systems running Tiebreaker 1.5 and later, run OpenJDK 17, 18, or 19. 
...
.Steps
. Download the MetroCluster Tiebreaker software and `MetroCluster_Tiebreaker_RPM_GPG` key.
+
The `MetroCluster_Tiebreaker_RPM_GPG` key is available from the same page where you download the software package for Tiebreaker 1.5 or 1.4 on the NetApp Support Site. 
+
