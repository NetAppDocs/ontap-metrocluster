...
* You must have installed OpenJDK by using the `yum install java-x.x.x-openjdk` command. For systems running Tiebreaker 1.5 and later, run OpenJDK 17, 18, or 19. The outputs in the example show JRE 1.8.0. (JRE 8).
...
. Verify the successful installation by running the following command in the `root` directory:
+
[source,cli]
# netapp-metrocluster-tiebreaker-software-cli
+
A successful installation displays the following prompt:
+
----
...
. Verify that you are running Tiebreaker 1.5:
+
----
...
. Verify that you are running Tiebreaker 1.4:
+
----
[root@mcctb ~] # netapp-metrocluster-tiebreaker-software-cli
NetApp MetroCluster Tiebreaker :> version show
