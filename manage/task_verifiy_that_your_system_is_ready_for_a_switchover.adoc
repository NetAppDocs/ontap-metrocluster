...
[.lead]
You can use the -simulate option to preview the results of a switchover operation. A verification check gives you a way to verify that most of the preconditions for a successful run are met before you start the operation. The switchover operation is initiated from the site that will remain up and operational.
