...
You can view the AUSO status in the `metrocluster show` command. Beginning with ONTAP 9.13.1, the default value is set to "`auso-on-dr-group`". For ONTAP 9.12.1 and earlier, the default value is set to "`auso-on-cluster-disaster`".
...
Beginning with ONTAP 9.13.1, the default value for the AUSO status is set to “auso-on-dr-group”. For ONTAP 9.12.1 and earlier, the default value is set to “auso-on-cluster-disaster”. In an eight-node MetroCluster IP configuration, this option will trigger an AUSO if both nodes in an HA pair fail. 

You can change the AUSO domain to the “auso-on-cluster-disaster” domain using the `metrocluster modify -auto-switchover-failure-domain auso-on-cluster-disaster` command. 
