...
NOTE: After the NSO is complete, you can manually bring unmirrored aggregates online.
a|
...
Beginning with ONTAP 9.13.1, the default value for the AUSO status is set to “auso-on-dr-group” in a MetroCluster IP configuration. For ONTAP 9.12.1 and earlier, the default value is set to “auso-on-cluster-disaster”. In an eight-node MetroCluster IP configuration, this option will trigger an AUSO if both nodes in an HA pair fail. 
