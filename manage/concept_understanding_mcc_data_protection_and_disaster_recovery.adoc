...
image::../media/mcc_hw_architecture_local_ha.gif["Local HA in a MetroCluster configuration"]
...
image::../media/mcc_mirroring_of_svms.gif["MetroCluster mirroring of SVMs"]
...
image::../media/mcc_svms_after_switchover.gif["MetroCluster SVMs after switchover"]
...
h| Type of switchover h| MetroCluster FC configuration state h| MetroCluster IP configuration state
...
image::../media/mcc_mirroring_of_pools.gif["MetroCluster mirroring of storage pools"]
...
a|
image:../media/mcc_nvram_quartering.gif["Nonvolatile cache quartering in MetroCluster configuration"]

a|
image:../media/mcc_nvram_split_in_non_mcc_ha_pair.gif["Nonvolatile cache split in non-MetroCluster HA pair"]
...
image::../media/mcc_nvram_mirroring_example.gif["MetroCluster NVRAM mirroring example"]
...
image::../media/mcc_nvram_mirroring_example_dynamic_dr_aux.gif["MetroCluster NVRAM mirroring with DR auxiliary"]
...
+
In a four-node MetroCluster configuration, this failure might lead to an automatic or a negotiated takeover of the impaired node, depending on the component that failed. Data recovery is described in link:https://docs.netapp.com/us-en/ontap/high-availability/index.html[High-availability pair management^] .
+
...
+
link:https://docs.netapp.com/us-en/ontap/high-availability/index.html[High-availability pair management^] has more information about how to recover from site-wide controller failures that do not include controller failures, as well as failures that include one or more controllers.
...
NOTE: AUSO (automatic unplanned switchover) is not supported on MetroCluster IP configurations.
...
link:https://docs.netapp.com/us-en/ontap/high-availability/index.html[High-availability pair management^]
...
image::../media/mcc_disk_ownership.gif["MetroCluster disk ownership"]
...
image::../media/mcc_so_sb_with_autoheal.gif["MetroCluster switchover and switchback with autoheal"]

=== Automatic healing after unscheduled switchover (beginning with ONTAP 9.6)

Automatic healing after an unscheduled switchover is supported on MetroCluster IP configurations beginning with ONTAP 9.6. An unscheduled switchover is one in which in you issue the `switchover` command with the `-forced-on-disaster true` option.
...
image::../media/mcc_uso_with_autoheal.gif["MetroCluster unscheduled switchover with autoheal"]
...
// 2023 Jan 22, GH issue 298
// 2023-APR-20, BURT 1532596
// 2023-FEB-28, ONTAPDOC-719
// 2022-JAN-20, BURT 1448684 
