...
[options="header"]
|===
| Failure type | DR mechanism (Four-node configuration) | DR mechanism (Two-node configuration) | Summary of recovery method (Four-node configuration) | Summary of recovery method (Two-node configuration)
|
Single-node failure
|
Local HA failover
...
2.3+|
After the node is restored, manual healing and switchback using the metrocluster healing and metrocluster switchback commands is required.
+
...
link:../manage/task_heal_the_configuration_choice_task.html[Healing the configuration]
