...
[.lead]
When the system is in switchover, NetApp recommends that you avoid performing certain operations. Learn about the operations that are not recommended when the system is in switchover. 

The following operations are not recommended when the system is in switchover which means that you should only perform them under guidance from NetApp: 

* Creating or deleting aggregates
* Creating or deleting volumes (including encrypted volumes) 
