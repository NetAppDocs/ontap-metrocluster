...
[.lead]
When the system is in switchover, NetApp recommends that you avoid performing certain operations. Learn about restricted operations when the system is in switchover. 

== Restricted operations in switchover

The following operations are restricted when the system is in switchover which means that you should only perform them under guidance from NetApp support: 
