...
..	If validation fails, and Site B is down, then there is most likely a connection problem. Verify that Site B is down, then perform the steps in link:https://docs.netapp.com/us-en/ontap-sm-classic/online-help-96-97/task_performing_unplanned_switchover.html[Performing an unplanned switchover^].
